...
- **The key is never shipped to the browser (on the Cloudflare worker).** The worker's `/api/config` no longer returns `googleApiKey`; it returns **`drivePublicPreview: boolean`** (`!!GOOGLE_API_KEY`) so the client's read ladder knows whether to try the proxy rung. [`drivePublicRead.ts`](../../packages/axoview-app/src/services/drive/drivePublicRead.ts) rung 1 now fetches the proxy (gated on `drivePublicPreview`, skipped `afterGrant`); rung 2 (token read, direct to Google) is unchanged. The **Express/Docker** target has **no** proxy, so it emits `drivePublicPreview: false` (never the key) and anonymous preview is simply off there — the read proxy is a Cloudflare-only feature ([routes.js](../../packages/axoview-backend/src/routes.js) `getConfig`).
- **Benefits:** no referrer allowlist, viewer never authenticates for a public link, resilient to client-side Google/COOP changes. (Caching is browser-side only — see the correction below.)
...
**2026-07-14b — the proxy honors Drive's `trashed` flag + a cache-scope correction.** The two-account gate surfaced that "delete" in Axoview is a Drive **trash** (soft, ~30-day recoverable — [ADR 0036 §3](0036-google-drive-storage-provider.md)), and a trashed file keeps its `anyone` permission, so the API-key proxy kept serving a "deleted" diagram — diverging from Drive's own web-share, where trashing kills the link. Fixed: the proxy now reads **metadata first** (`fields=trashed,size`) and returns **`410 Gone`** for a trashed file; the client maps 410 → terminal `not-found` (the `ReadonlyLoadErrorDialog` "Could not open … may have been deleted", **not** the sign-in ladder). Net: **delete → the link dies; restore from Trash → it revives**, matching Drive semantics, with zero permission mutation. The metadata read also supplies `size` for the 10 MB cap (replacing the content-length check). Correction to the paragraph above: Cloudflare does **not** edge-cache Pages-Function responses without a Cache Rule, so `Cache-Control: 60s` is **browser-side dedupe only** — each distinct viewer still hits Drive, so a per-caller rate limit remains the [ADR 0043 §4](0043-deferred-backend-for-google-api-hardening.md) follow-up if abuse appears.
