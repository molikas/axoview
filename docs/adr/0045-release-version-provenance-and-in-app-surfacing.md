...
3. **Nearest tag + distance + short sha** (`git describe --tags`) → e.g. `3.8.1-3-gabc1234`, an honest "preview past the last release" marker.
4. **Fallback** to the passed `package.json` version when git/tags are unavailable — no worse than today.

This needs **no push to master, no ruleset change, and no release-pipeline change** — it reads the tag semantic-release already creates.
...
**Negative / risks:**
- **One-release lag / preview suffix.** Cloudflare builds a commit *before* its release tag exists (the tag is created by the later Release job), so a production build typically resolves to the *previous* tag or a `X.Y.Z-N-gsha` marker rather than the exact just-cut version. This is a large improvement over a multi-release freeze, but it is not exact. **Optional exactness enhancement:** trigger a Cloudflare deploy hook from the Release job *after* the tag is created, so the rebuild sees the exact tag — deferred (needs a CF deploy-hook secret; see Implementation notes).
- **Tags must be present in Cloudflare's build clone.** If CF's clone omits tags, resolution falls back to the frozen `package.json` (status quo). Verify against the live CF build; add a `git fetch --tags` build step if needed.
...
- Exactness enhancement (deferred): in [release.yml](../../.github/workflows/release.yml), after `npx semantic-release`, add a secret-guarded `curl -X POST "$CF_DEPLOY_HOOK"` step so Cloudflare rebuilds the now-tagged commit. No-op when the secret is unset.
...
- **Manual verification (surface):** a build whose `HEAD` is a release tag shows that version in the About tab and the `#ax-splash` boot stamp (e.g. `3.8.1`, not `3.7.0`); a build ahead of the tag shows the `X.Y.Z-N-gsha` preview marker; a freshly exported project zip's exporter tag matches.
