...
> **Amendment (2026-07-06, granular consent — shipped in `41fe523`, documented here 2026-07-07):** Google's consent screen lets a user sign in while leaving the Drive checkbox unchecked, so the granted scopes can be less than the requested ones. The store therefore tracks **`driveScopeGranted: boolean | null`**, derived from the token response's `scope` list (`null` = no token yet; a scope-less response — older GIS shapes, tests — is treated as granted rather than false-alarmed). **As-built:** a scope-less grant does **not** yield a working identity-only session — `authStore._onToken` hard-stops: it discards the token, calls `clearProfileHint()`, and parks in a `DRIVE_ACCESS_REQUIRED` state (`grantDriveAccess()` / `markDriveScopeMissing()` drive recovery). That state is absent from this ADR's typed `AuthStatus`; the "identity works either way, UI just gates on the flag" framing is aspirational, not what shipped. Covered by `authStore.test.ts` (see [testing.md](../guidelines/testing.md)).
