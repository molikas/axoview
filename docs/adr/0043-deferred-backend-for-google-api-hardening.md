...
- **#3 mechanics:** the Worker holds the Drive API key / OAuth creds server-side,
  streams `files.get?alt=media` back to the browser, and adds a cache +
  rate-limit; the public API key is then removed from the client bundle.
