...
> **Status:** Artifact frozen 2026-05-23 (Sessions A + B + C complete). Corrections appendix added 2026-05-25. Reviewer-ready. This document is a snapshot of the post-M10 v1.0.0 state — it does not track ongoing changes; the [ADRs](../adr/), [PLAN.md](../../PLAN.md), and [docs/guidelines/architecture.md](../guidelines/architecture.md) are the living artifacts.
...
**Snapshot date.** This artifact is dated **2026-05-23** and is **not a living doc**. It captures the post-M10 state of the [productization audit](../tactical/productization-audit.md) (v1.0.0 shipped 2026-05-23). The audit itself, the ADRs, [PLAN.md](../../PLAN.md), and [docs/guidelines/architecture.md](../guidelines/architecture.md) are the living artifacts; this one is a frozen review surface.
...
|---|---|---|---|
| **App shell** | `App.tsx` was 745 lines (intermixed state / effects / JSX) | `App.tsx` is 103 lines (pure provider composition) | [Phase 0A](../../PLAN.md), [architecture.md §2l](../guidelines/architecture.md#2l-axoview-app-provider-decomposition-2026-04-27) |
...
| **Surface vocabulary** | Dialog / Modal / Popover used interchangeably | Six locked terms: Dialog · Modal · Popover · Panel · Banner · Screen | [ADR 0008 D2](../adr/0008-naming-convention.md#2-modal-vs-popover-vs-dialog-vs-panel--locked-vocabulary), [UX §1](../guidelines/ux-principles.md#1-layout) |
...
The lib carries four Zustand stores; the app carries one more (notifications). All five use the React-context-wrapped Zustand pattern (per-mount-instance isolation) — see [architecture.md §2a](../guidelines/architecture.md#2a-store-layer).
...
The 11-mode state machine — formal definitions in [architecture.md §2b](../guidelines/architecture.md#2b-mode-state-machine).
...
Everything quantifiable about the v1.0.0 state, in one place. Numbers come from existing sources ([§7.9](#79-inventory-totals), [`docs/guidelines/testing.md`](../guidelines/testing.md), [`git-automation-hardening.md`](../tactical/git-automation-hardening.md), measured CI output); this section's value is putting them on a single page for the reviewer.
...
The 10% threshold is intentionally low while the suite grows; raising it is a polish-wave item, not a productization gate. Current global statement coverage on `axoview-lib` is ~32% per [`docs/guidelines/testing.md`](../guidelines/testing.md#code-coverage) — comfortably over the threshold but well below "comprehensive."
...
13. **Test architecture — over-mocked surfaces.** [`docs/guidelines/testing.md`](../guidelines/testing.md) labels suites VALID vs SEMI-VALID; SEMI-VALID means a local copy of a production constant is asserted (the production module can't be imported in Jest). Walk the SEMI-VALID list. For each, is the divergence risk worth the test, or is the test actively misleading?
...
## 12. Corrections (added 2026-05-25)

This appendix corrects specific claims made in §1–§11 above, surfaced during v1.1-tech-debt Tracks 0–2 verify-grep passes. Per the frozen-artifact convention §1–§11 are not edited in place; corrections live here so the diff between "what the artifact said" and "what is actually true" is reviewable in one place. Each subsection cites the original §/row it corrects.

**B2 (corrects [§3b](#3b-package-responsibilities) and [§7.4](#74-packagesaxoview-worker)):** §3b's worker paragraph and §7.4's prose both state that `axoview-worker` "imports `routes.js` from `axoview-backend` directly (the cross-package import is the single source of truth)." In the current tree this is false at runtime — [`packages/axoview-worker/src/app.ts`](../../packages/axoview-worker/src/app.ts) imports only `hono`, `hono/body-limit`, `hono/secure-headers`, and `./auth`; it does not import `routes.js`. The cross-package reference exists exclusively in [`packages/axoview-worker/tsconfig.json`](../../packages/axoview-worker/tsconfig.json) via `"include": ["src/**/*", "../axoview-backend/src/**/*"]`, which is a typecheck-only coupling. The Worker re-implements `/api/config` inline and short-circuits every other `/api/*` route to 503; there is no shared-route execution path today. Reviewers: treat §3b's and §7.4's "single source of truth" wording as aspirational — the typecheck-include is the only artifact of that intent; runtime coupling will only exist if/when Phase 3B reintroduces storage routes on the Worker.

**B5 (corrects [§5d](#5d-env-var-contract-per-target) and [ADR 0009 §5d](../adr/0009-deployment-topology.md)):** §5d's self-host env-var table and the matching ADR 0009 §5d row both state `ENABLE_SERVER_STORAGE` "defaults to `false`. The implicit-off default is intentional — a fresh container without this flag set behaves as if it were a static-only deploy and the SPA falls back to Local mode." That description applies to the bare Node process, not the shipped container. [`Dockerfile:55`](../../Dockerfile#L55) sets `ENV ENABLE_SERVER_STORAGE=true`, so the container default is the opposite of the documented behaviour. Per v1.1-tech-debt DP2-B5(b) this is the *correct* posture for Docker (operators choosing the container overwhelmingly want server-backed storage; the implicit-off default is right for the non-Docker SaaS / `npm run dev` path but inverted for the container image) and the resolution is documentation, not a Dockerfile flip. Reviewers: when reading §5d and ADR 0009 §5d, mentally split the `ENABLE_SERVER_STORAGE` row in two — `false` for bare-Node/SaaS, `true` for the Docker image.

**M6 (corrects [§6d](#6d-csp--security-headers)):** §6d's verification column for the Worker layer says Hono `secureHeaders()` "Matches the static set; CI verifies neither layer drifts." There is no such CI step. [`.github/workflows/test.yml:66`](../../.github/workflows/test.yml#L66) verifies only that `packages/axoview-app/build/_headers` exists as a build artifact; it does not diff the static `_headers` set against the headers Hono's `secureHeaders()` emits, nor against the headers in [`nginx.conf`](../../nginx.conf). The three layers (Cloudflare static `_headers`, Worker `secureHeaders()`, nginx) are kept aligned by hand. Reviewers: treat ADR 0009 D5's "divergence between layers is a bug" as an operator-discipline rule, not a CI-enforced invariant. A future polish-wave item could add a generation/diff step (e.g., a script that emits the canonical header set and compares it against all three sites).

**M7 (corrects [§8a](#8a-test-inventory)):** §8a's coverage-threshold row reads "10% (global lines/branches/functions/statements) — Hard-fail gate per T2 G2" without scoping. The 10% threshold is configured *only* in [`packages/axoview-lib/jest.config.js`](../../packages/axoview-lib/jest.config.js#L36-L43). [`packages/axoview-app/jest.config.js`](../../packages/axoview-app/jest.config.js) declares no `coverageThreshold`; [`packages/axoview-backend/`](../../packages/axoview-backend/) and [`packages/axoview-worker/`](../../packages/axoview-worker/) carry no jest configuration at all (already named in [§7.8](#78-cross-package-observations)). The gate covers `axoview-lib` (~63% of source LOC per §8c) and nothing else. Reviewers: when reading the §8a row and the T2 G2 reference, treat the 10% threshold as a lib-only floor; app/backend/worker coverage is unconstrained by CI today.

**Anomaly #31 (corrects [§7.2 row at `useRuntimeConfig.ts`](#72-packagesaxoview-app), audit anomaly):** §7.2's row for [`useRuntimeConfig.ts`](../../packages/axoview-app/src/hooks/useRuntimeConfig.ts) flags the `serverStorage` field as dead — "plumbed through worker/backend but mode detection now reads `LocalStorageProvider.usingServer` instead." Track 0f verify-grep confirmed the opposite: [`packages/axoview-app/src/providers/AppStorageContext.tsx:47`](../../packages/axoview-app/src/providers/AppStorageContext.tsx#L47) calls `manager.setServerStorage(config.serverStorage)`, which is what subsequently sets `LocalStorageProvider.usingServer`. The field is the upstream source for the boolean, not a dead alternative path. Per ADR 0009 D2 the single `/api/config` probe and the `serverStorage` field it returns are the canonical mode signal. Reviewers: the field is live and load-bearing; the audit row was wrong.

**Published-posture flag (calls out [§7.1 row 810](#71-packagesaxoview-lib) explicitly):** [`packages/axoview-lib/package.json`](../../packages/axoview-lib/package.json) carries `"private": true` (per [Locked Decision #11](../tactical/productization-audit.md#locked-decisions-from-scoping-discussion-2026-05-19) the lib is monorepo-only, not npm-published) but advertises `"main": "./dist/index.js"`, `"types": "./dist/index.d.ts"`, `"files": ["dist"]`, and a `prepublishOnly` script — i.e., the package retains a publishable shape behind the private flag. §7.1's row already names this as "intentional but worth a reviewer flag." Surfacing it here so the reviewer encountering the row in the inventory has the explicit context: do not propose adding/removing the `main`/`types`/`files` fields, and do not propose flipping `private: false`. The shape is preserved deliberately so that a future decision to publish is a one-flag change, not a re-author of the manifest.

**Auto-generated artifacts a reviewer should not red-pen (surfaces [§7.8 bullet 10](#78-cross-package-observations)):** Surfaced here for reviewer convenience because LOC summaries elsewhere in the artifact include these files in raw counts. `CHANGELOG.md` (root, appended by `@semantic-release/changelog`); the three `package-lock.json` files (root, `packages/axoview-app/`, the dead `packages/axoview-lib/docs/`); and every `__snapshots__/*.snap` under `packages/axoview-lib/src/components/DebugUtils/__tests__/` are all auto-generated. Reviewers: skip these in any "this file looks weird / could it be smaller" pass; the next maintenance touch will overwrite anything edited here.

**§7.1 row 810 (corrects version string):** Artifact records axoview-lib version as `v2026.5.21`; current [`packages/axoview-lib/package.json`](../../packages/axoview-lib/package.json) is `2.0.0` (semantic-release auto-bumped to `2.0.0` on the v1.1 Track 0 squash via the MainMenu BREAKING CHANGE footer; see [Locked Decision #11](../tactical/productization-audit.md#locked-decisions-from-scoping-discussion-2026-05-19) + Track 0 release notes). Reviewers: the `v2026.5.21` reference is a snapshot of the date-stamp era pre-v1.0.0 release; treat as historical.
