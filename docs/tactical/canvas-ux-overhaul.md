...
- [ ] **LayersPanel open/select** — row single-click selects, double-click opens the panel (mirror canvas; §4.1). **Preserve the T3 identity-stable handler pattern** (commit c875b652: churny `mode`/`itemControls` closures read from a ref so `onClick` stays empty-dep) — add the new `onDoubleClick` the same way, or T3's O(N)-re-render regression returns.
