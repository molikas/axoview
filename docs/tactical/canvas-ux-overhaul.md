...
### T4 — Interaction model (ADR 0022 + ADR 0027)

**Session A shipped** (commit 7f01645 — open/select split): #4, #6, action-bar-on-selection, LayersPanel open/select, and the `onContextMenu` no-longer-mounts-panel half of #7. Unit-pinned (multiSelect.contract); **browser + touch walkthrough still owed** before promotion. Session B = context menu + right-tap/long-press wiring + customization removal.

- [x] **#4** single-click selects only; double-click opens the Properties panel — store select-only path (`setSelectedIds(len 1)` + `setItemControls(…, {openPanel:false})`), mouse body `dblclick` + touch double-tap open the dock ([Cursor.ts](../../packages/axoview-lib/src/interaction/modes/Cursor.ts), [useInteractionManager.ts](../../packages/axoview-lib/src/interaction/useInteractionManager.ts), [uiStateStore.tsx](../../packages/axoview-lib/src/stores/uiStateStore.tsx)).
- [~] **#7** right-click **drag** = pan (already worked); `onContextMenu` no longer mounts the panel/bar for mouse — **done**. Right-click **tap** = context menu — **Session B** (needs the menu component).
- [ ] **Build the context menu (ADR 0027)** — item + empty-canvas variants; command catalogue (incl. unsnap/collision from T8, z-order, assign-layer); right-tap + long-press invocation; division of labor with NodeActionBar (no command reachable only via a removed gesture).
- [x] **#6** panel no longer closes during an in-panel name text-drag — `Cursor.mouseup` gates the clear on `mousedownHandled` (set only by a renderer-side press), not the `mouse.mousedown` snapshot `getMouse` records for any press.
- [x] **action bar on selection** (opens on single-click/tap via the store select-only path); **Alt+click** without pre-select already shipped in T1 (#1).
- [x] **LayersPanel open/select** — row single-click selects, double-click opens the panel; rename stays F2. Preserves the T3 identity-stable handler pattern (`onClick`/`onOpen` empty-/stable-dep, commit c875b652).
- [ ] **Remove customization surface** — delete `PanSettings`, the Settings "Canvas" pan section, the `panSettings` slice/action/persist/prop, 14× `settings.pan.*` i18n, `usePanHandlers` branches; collapse hotkeys to one fixed read-only scheme; rewrite `HelpDialog`; clean dead persisted keys on load.
- [~] **Touch** — tap=select, **double-tap=details** shipped (`DOUBLE_TAP_MS` debounce in the touch machine); **long-press=context menu** (reassigned from the action bar) is **Session B**; verify on a device (ADR 0018).

### T9 — Present mode (extend ADR 0013)
- [ ] **#15 hide-labels control** — ephemeral UI-only toggle in the presentation chrome (mirror `PreviewLayerSwitcher`; never dirties save).
