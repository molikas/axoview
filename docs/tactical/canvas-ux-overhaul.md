...
### T5 — Export robustness & presets (ADR 0025) — **code complete**

Lib jest green (1155) + both libs build clean. **Owed before promotion:** run the new `import-export-image.spec` (needs a browser) + a manual export walkthrough (SVG opens valid · 4× large diagram · Screenshot preset labels).

**Walkthrough follow-ups (2026-06-20)** — fixed: (a) the export snapshot was capturing the editor chrome (left/right docks + the bottom zoom/Aa/help cluster) because only `UiOverlay`, not the three docks, gated on NON_INTERACTIVE — now the whole chrome is gated off for snapshot renders (export + thumbnails); (b) a right-click on the export Dialog opened the canvas context menu — the window-bound right-tap (ADR 0027) now only arms when the press lands on the renderer (`usePanHandlers`, pinned); (c) Show-grid defaults on; (d) the browser-compat notice renders only on Firefox.

- [x] **#9 SVG throws** — reproduced: `btoa(unescape(encodeURIComponent()))` in [svgOptimizer.ts](../../packages/axoview-lib/src/utils/svgOptimizer.ts) threw `URIError` on lone surrogates (broken-paste names) and used deprecated `unescape`. Replaced with a `TextEncoder`-based [`utf8ToBase64`](../../packages/axoview-lib/src/utils/svgOptimizer.ts) (U+FFFD substitution, never throws); `optimizeSvgDataUrl` now degrades to the original (valid) data-URL on any encode/decode failure. Residual failures surface in-dialog (ADR 0011 §1 in-dialog-inline carve-out — the existing `exportError` Alert). Unit-pinned: Unicode round-trip + lone-surrogate no-throw.
- [x] **#18 4× canvas limit** — one shared pure [`computeRenderTarget`](../../packages/axoview-lib/src/utils/renderTarget.ts) (bounds + requested scale + browser caps → effective scale + dims + `wasClamped`) used by **both** `exportAsImage` and `exportAsSVG`; the dialog surfaces a `wasClamped` warning Alert with the achievable size (never a silent blank). Unit-pinned: under-limit passthrough · over-dimension clamp · area clamp · edge cases.
- [x] **#19 labels + Screenshot preset** — `RendererProps.readableLabels` (ADR 0015 counter-scale, keeps labels legible at the fit-to-view zoom) + `RendererProps.showLabels` (visibility) wired through Axoview to the export-scoped `uiState.exportHideLabels`, merged at the 3 name-label render sites (Node · ConnectorLabel · NodesCanvas) alongside `previewHideLabels`. Dialog adds an explicit **Show labels** option + the **Screenshot (recommended)** preset (2× · fit · labels on · PNG) as the default selection. Persist effect now skips NON_INTERACTIVE instances so the hidden export Axoview can't leak its label flags into the live canvas's settings. i18n × 13.
