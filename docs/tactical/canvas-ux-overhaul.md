...
- [ ] **#4** single-click selects only; double-click opens the Properties panel ([Cursor.ts:465,575](../../packages/axoview-lib/src/interaction/modes/Cursor.ts#L465)).
- [ ] **#7** right-click **drag** = pan; right-click **tap** = context menu (not details/bar); `onContextMenu` no longer mounts the panel for mouse ([useInteractionManager.ts:960](../../packages/axoview-lib/src/interaction/useInteractionManager.ts#L960)).
...
- [ ] **#6** reproduce + fix the panel closing during an in-panel name text-drag (instrument; window-bound listeners per ADR 0018).
- [ ] **action bar on selection**; **Alt+click** without pre-select (shared with T1 #1).
- [ ] **LayersPanel open/select** — row single-click selects, double-click opens the panel (mirror canvas; §4.1).
...
- [ ] **Touch** — tap=select, double-tap=details, **long-press=context menu** (reassigned from the action bar); verify on a device (ADR 0018).
...
- [ ] **Focus the A/B on the paths this program touches:** lasso hit-test per drag frame (T2 — runs every marquee frame; must not regress the `getItemsInBounds` cost), layers-panel @1000 (T3 — expect improvement, measured), grid offset render + drag CSS-preview (T8), label drag (T6). One variable per iteration; never bundle.
...
| T3 layers panel @1000 (#14) — **the fix** | **Improvement, measurable** — carries its **own** same-session before/after (click→panel-open latency, median-of-N), not the frame harness | virtualize + memoize `LayerItemRow` | no latency drop = the fix didn't land; **canvas spawn p95 must stay flat** (the panel is a separate React tree — a regression there means the change leaked) |
