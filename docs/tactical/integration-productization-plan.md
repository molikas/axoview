...
>
> **Status:** In progress — Slice 0 + A + B + C1 + E + G1 landed, plus a **beyond-plan UX slice** (connector decouple, placement cancel, Notes-on-all + Metadata deck, floating-Label edit, unified label sizing); **D1 + D2 + A5 + F2 + RT (rich-text dedupe, ADR 0034) + RT-2 (Lucid-parity polish) + F1 (toolbar overflow) done — the must-ship list is CLEAR; RT-3 (Lucid field-test finds) is implemented locally awaiting owner validation; /ship gates (review + doc sync) are next** · **Owner:** molikas · **Last updated:** 2026-07-03
>
...
- ✅ **RT — Rich-text popup retirement / inline canvas text editing DONE (2026-07-03).** [ADR 0034](../adr/0034-inline-canvas-text-editing-and-dual-scope-strip-formatting.md): the strip's "Rich text" popover is deleted; text boxes edit inline on canvas (toolbar-less Quill, promoted above the interactions box while editing; double-click/F2/Rename/place-and-type; click-away commits, Escape cancels, Enter = newline) and format from the strip's **dual-scope B/I/U/S + lists** cluster (element booleans for label types; content HTML for text boxes — whole content selected / live range editing; bulk text boxes now format, closing the old dead-end). Also landed: connector-label `fontSize` schema cap 24→40 (the strip could brick saved diagrams — S1), dead `nameLabel*`/`'__name__'` strip branches deleted (cross-type stepper now steps `labels[].fontSize`), legacy `textBox.is*` flags folded into content at load, bulk node Link fan-out fix, strip show/hide eye extended to connectors, Link text-range mode, tooltip copy fixes, `richText*` i18n keys purged + 7 new keys across all 13 locales. Verified: 141 jest suites, lib+app builds, **14 e2e green in Chromium** (rewritten `textbox-text-edit-move` incl. new commit/cancel cases + bulk-style + J5 + lasso + textbox-ops + shapes). **F1 note: re-measure the overflow breakpoint against the new strip (net width ≈ unchanged: +U toggle +2 list buttons −Rich-text button).** Open ADR 0034 TODOs for the owner: label-type underline fields (O1), cross-type text color (O3), non-connector pre-draw defaults (O6).
- ✅ **F1 — Toolbar overflow fix DONE (2026-07-03).** [ADR 0005 §7 amendment](../adr/0005-toolbar-and-dock-layout-contract.md): the Group-1 style slot is the ONE compressible group — the right cluster may shrink (`flexShrink:1; minWidth:0`) and the slot absorbs the squeeze with internal horizontal scroll (thin scrollbar), so Save/Export/Share/Present/sidebar-toggle keep natural width and stay reachable at any viewport width. Breakpoint verification = new `toolbar-overflow.spec.ts` (walks 1280→800px, asserts Group 3+4 fully on-screen + the squeeze lands on the slot); new ids `toolbar-style-slot` / `toolbar-sidebar-slot`. Same slice also resolved ADR 0034's O1/O3/O6 owner TODOs: element-level **underline** for all three label types (schema quads + both Canvas2D paths + both DOM paths + strip U everywhere B/I/S works), cross-type mixed-selection **text color**, and neutral armed-tool disabled tooltips (`armedToolPlaceFirst`); +2 i18n keys × 13 locales.
- ✅ **RT-2 — Lucid-parity text-editing polish DONE (2026-07-03, owner: "regular rich text editing like Lucid").** [ADR 0034 dated addendum](../adr/0034-inline-canvas-text-editing-and-dual-scope-strip-formatting.md): the three renderings of one `content` (live Quill editor / resting canvas / isoMath measurement) now share ONE formatting geometry — new `textBox.lineHeight` multiplier (default **1.2** = Lucid, strip **Line spacing** slider in the Text-size popover, p/li margins zeroed so the multiplier is the only knob) + one list-indent constant (`CANVAS_RICHTEXT_LIST_INDENT_EM` = 1.5em) in all three consumers, with per-`<li>` indent finally **measured** (kills the "one character per line" bulleted box). **Markdown list autofill restored** (`- `/`* `/`1. ` → list; MQA #12 retired — shared `quillListAutofill` binding in both rich surfaces, Ctrl+Z restores the literal, checkboxes excluded). Editor **grows as you type** (`max-content`); the stale dashed selection box is suppressed mid-edit. +`lineSpacing` i18n key × 13 locales; jest 142 suites green (new autofill behavioral suite + re-pinned isoMath weights).
- 🧪 **RT-3 — Lucid field-test finds (2026-07-03/04, IMPLEMENTED LOCALLY — uncommitted, owner validating before push).** From the owner's Lucid testing notes ([ADR 0034 second + third 2026-07-03/04 addenda](../adr/0034-inline-canvas-text-editing-and-dual-scope-strip-formatting.md)): **manual size + soft wrap** (optional `textBox.width` + `height` (min-height), full rectangle-style 8-anchor set → new `TEXTBOX.TRANSFORM` mode, greedy-wrap height estimate, "Fit to text" context-menu reset); **empty-box lifecycle** (new boxes born empty + localized placeholder; a session ending empty deletes the box — no more ghost elements); **paragraph alignment** (left/center/right, dual-scope, inline `text-align` via Quill style attributor); **paste normalization pinned** by a synthetic-ClipboardEvent e2e (no code change needed). Owner round-2 fixes (2026-07-04): **resting wrap root cause** — Quill `getSemanticHTML` serializes every space as `&nbsp;` → `normalizeQuillHtmlSpaces` on commit+load + resting white-space contract (auto `pre`, fixed `pre-wrap`/`minWidth:0`); **dual-scope text color** (range via bridge lastRange, `formats` += color); **direction toggle moved into the text cluster**; **range-Link URL field** local-draft (was a stale-controlled field eating keystrokes). Owner round-3 (2026-07-04): **iso-Y multi-row placement fix** (originOverride at the far row — rows painted outside the selection) + **direction flip resets manual size**; **resize anchors stay visible during the edit session** (anchor press commits first); **`textBox.backgroundColor`** (strip Background gains the text-box branch); **ONE Lucid-style alignment control** — 3×3 grid combining dual-scope horizontal × new element-level `verticalAlign`. Owner round-4 (2026-07-04): **live session frame** (new UI-store `editingTextBoxSize` measured with the exact commit pipeline — container + transform bounds track typing, empty box measures its placeholder) + **transparent chrome-less editor** (white sheet/outline/extra padding gone; editing == resting); **Ctrl/Cmd+K link shortcut everywhere** (canvas Quill binding with Docs-style caret→word expansion → strip Link popover via StripButton `openEvent`; window listener for selected node/connector/Label; Label inline-edit contentEditable; deck editors keep snow's native binding) + **spurious collapsed-selection guard** (bridge records ranges only while the editor owns focus; strip reads `getEffectiveEditorRange`); **alignment re-cut with standard MUI icons** (two independent rows — `FormatAlign*` / `VerticalAlign*` — replacing the unreadable hand-rolled 3×3 composites; no per-button tooltips, they swallowed the second row's clicks); **file-explorer empty-space right-click** (app-side: targets the tree-selected node, else the open diagram — no more browser menu). Owner round-5 (2026-07-04, PUSHED with rounds 1-4 as e53ab99 + follow-up): **Docs-parity link UX** — `TextBoxLinkCard` (caret-in-link or hover → body-portaled Popper chip at the link's post-transform rect: URL open/copy/edit-inline/remove; blot-based extent), Enter in the strip URL field applies + CLOSES the popover (StripButton `closeEvent`) + lands the caret in the link so the card confirms, `normalizeWebLinkUrl` protocol forgiveness on text-link applies (blank = remove); two keystroke traps fixed (mount-blur phantom apply; sync-close-in-keydown landing Enter's default action in the refocused editor as a phantom leading `\n`); **file-explorer VS Code create block** — New diagram / New folder / Refresh on folder rows + empty space (folder rows create INSIDE + auto-expand, empty space creates at root; menu now opens even with no diagrams). +4 i18n keys (linkCopy/Copied/Edit/Remove) × 13 locales. Copy/Paste style stays parked. Jest 144 suites/1474 green; e2e 20/20 (textbox spec 11 incl. EDIT-BOUNDS + LINK + LINK-CARD, file-explorer 3, regression set 6). Round-5 follow-up (owner: "when you drag the text the resize box stays in the original place"): **selection chrome hides during DRAG_ITEMS** — the move is a CSS-only preview (RECT-1), so the bounds/anchors sat frozen at the ORIGIN tile for the whole drag while the item followed the cursor; TransformControlsManager now returns null in that mode (Lucid/Figma convention), all item types. Probe-verified mid-drag (anchors 0 while --ff-drag-dx applied) + post-drop commit clean; e2e 16/16 on the drag-affected set. Owner round-6 (2026-07-04): **Ctrl+K now opens the INLINE link card at the selection** (create prefills from an existing link; the strip popover stays the element-headerLink surface; third phantom-Enter trap fixed — never refocus Quill synchronously inside a keydown); **on-canvas rotate handle** for text boxes (iso-plane flip) AND rectangles (center transpose) — the strip's text-direction toggle DELETED (4 i18n keys retired → `rotate90` ×13, "de-dense the top control"); **text-box border** = the rectangle option set (style/width/color/opacity, CSS border on the shared container; absent color = no border; strip Border enabled for text boxes bulk-aware, seeds a default color on style-first picks; borderDisabled copy updated ×13). New `rotate-border.spec.ts` (3 tests). Jest 144/1474; e2e 23/23 (textbox 11, rotate-border 3, explorer 3, regression 6). Owner round-7 polish (2026-07-04): **rotate handle 32px + stock RotateRightOutlined** (was "too small and non-standard"); **mid-edit geometry writes re-measure the live draft** (font/spacing slider mid-session left the old footprint under the enlarged text — the live preview only refreshed on typing; TextBox keeps the last draft + re-measures on fontSize/lineHeight/width/height/orientation changes; EDIT-BOUNDS e2e extended); **link card Docs pass 2** — "Search or paste a link" ghost text (+1 key ×13) + diagram suggestions from `linkedDiagrams` (typing filters; picking writes an internal `#diagram:<id>` fragment href that survives Quill's whitelist + sanitizer; card shows the target NAME; resting clicks dispatch `axoview-navigate-to-diagram` — plain click navigates in view modes, Ctrl/Cmd+click in edit).
...
**Non-blocking loose ends (P3 — 2026-07-02 audit; bundle into a schema/i18n-tidy pass):** connector add-label **cap divergence** (F2 + context-menu writers uncapped vs schema `.max(256)` — add the guard to [`ConnectorLabel.tsx:288`](../../packages/axoview-lib/src/components/SceneLayers/ConnectorLabels/ConnectorLabel.tsx#L288) + [`CanvasContextMenu.tsx:167`](../../packages/axoview-lib/src/components/CanvasContextMenu/CanvasContextMenu.tsx#L167)); stale `nameLabel*` schema comment + the defensive `'__name__'` guard in `ConnectorLabel.tsx` + over-broad `ConnectorLabels` mount filter *(the strip's dead `'__name__'`/`nameLabel*` branches and the dead `textBox.is*` flags were RESOLVED 2026-07-03 by ADR 0034 — flags fold into content at load, strip writes `labels[].fontSize`)*. *(Panel-structure asymmetry RESOLVED 2026-07-02 — all 5 panels unified on the collapsible-section deck.)*
