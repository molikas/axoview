...
>
> **Status:** In progress — Slice 0 + A + B + C1 + E + G1 landed, plus a **beyond-plan UX slice** (connector decouple, placement cancel, Notes-on-all + Metadata deck, floating-Label edit, unified label sizing); **D1, D2, F1, F2 remain** (+ A5) before /ship · **Owner:** molikas · **Last updated:** 2026-07-02
>
...
- ⬜ **D1 — i18n TopBarStyleControls + dead-key purge (L).** Confirmed still outstanding (2026-07-02 audit): `TopBarStyleControls.tsx` has **no `useTranslation`** (~30 hardcoded strings) and ~28 dead locale keys (per-key purge — keep the still-live `connectorControls` keys). NB: the round-2 `rightSidebar` namespace is *not* D1; the strip is still English. **Must also cover the D2 Link-mode strings — sequence D2 first (or together) so the strip is localized in one pass, not twice.**
- ⬜ **D2 — Unified Link control: web URL + link-to-diagram (M).** *(Owner 2026-07-02: fold in here as a small slice, not a standalone ADR.)* Today the strip **Link** control edits `headerLink` (web) only; **diagram-to-diagram linking is alive but stranded** — `modelItem.link` + a working picker ([`NodeInfoTab.tsx:152-214`](../../packages/axoview-lib/src/components/ItemControls/NodeControls/NodeInfoTab/NodeInfoTab.tsx#L152-L214)) live node-only in the Details deck and are hidden unless `linkedDiagrams>0`. Make the strip Link control a **two-mode** control (Web URL | Link to diagram) writing `headerLink` vs `link`; extend diagram-link to connectors/labels for parity; retire the picker from `NodeInfoTab`. Small, high-value unification (same move as the fonts pass). **Feeds D1** (new strip strings).
...
- ⚠️ **F2 — Regression-test floor (M).** ✅ **`TextBox.ts`/`Label.ts` mode unit tests DONE (2026-07-02)** — arm-vs-place gating (arming tap creates nothing → no double-placement), exactly-one-create on a canvas release, drag-from-panel places, wrong-mode guard (12 tests, [`interaction/__tests__/TextBox.test.ts`](../../packages/axoview-lib/src/interaction/__tests__/TextBox.test.ts) + `Label.test.ts`). **Remaining:** right-click-cancel for TEXTBOX/LABEL/PLACE_ICON; the placement mode-hint pill + mouse ghost; `getAnchorOrdering` off-path fallback; node+connector Details/Notes panel-parity. Plus **fix the known-red** `multi-select-drag-lasso › rectangle AND textbox` test ([known_issues.md](../../known_issues.md)).
...
- ⬜ **A5 — ADR 0020 dated addendum** documenting the new perf scenarios (small; scenarios already live in the spec + `perf-results/e-slice-gate.md`).
- ✅ **P1 lasso e2e regression FIXED (2026-07-02).** The lasso-reset (`4315979`) left `multi-select-drag-lasso` + `lasso-connector-delete` helpers reading stale `mode.selection`; repointed both to the persistent `selectedIds`. 4/5 assertions restored; the 5th is the known-red mixed-marquee test above.
