...
- ✅ **D1 — i18n TopBarStyleControls DONE (2026-07-02).** New `topBarStyleControls` namespace (58 keys) in the `AxoviewTranslations` type + `en-US` + **all 12 non-English locales** (translated via a fan-out workflow); `useTranslation` wired into the strip + its 4 helper components, ~47 hardcoded strings → `t()` (incl. the D2 Link + opacity strings). **Zero hardcoded strings** in the strip; tsc-validated across all locales; i18n-completeness test + build + 7 strip e2e green. **Dead-key purge deferred** — pure cleanup, no runtime effect, doesn't block the AC or the completeness test.
