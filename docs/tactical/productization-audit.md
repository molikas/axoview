...
| B-9 | **SPA error UX polish.** Multi-file edit (likely `App.tsx` routes + `DiagramLifecycleProvider.tsx` error paths + a new generic error component). Observations: (a) Session share-viewer `/display/p/<uuid>` likely has the same silent-failure pattern B2 fixed for Local mode — needs parity treatment for bad/missing uuids; (b) SPA catch-all 404 route — verify presence; if absent, add a minimal 404 component; (c) generic API-error surface — when in-flight API calls from the SPA fail (network error, 5xx, unexpected 4xx), the SPA should surface a toast or dialog rather than silent state corruption. **Note:** T1 (E2E rewrite) will inform the exact scope — defer execution until after T1's scenario catalog clarifies which error UX behaviours need to be explicit contracts vs nice-to-have polish. | App.tsx + DiagramLifecycleProvider.tsx + new error component | B-7 smoke (2026-05-22) | low-med | bundle with B-1..B-4 cleanup wave (gated on T1) |
