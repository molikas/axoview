...
| B-3 | **[x] `95a0fd5`** Removed the custom document-mousedown outside-click listener in [AppToolbar.tsx](../../packages/axoview-app/src/components/AppToolbar.tsx) that closed the share popover whenever the click target wasn't inside the share button's parent. MUI Popover renders into a portal, so clicks on the share URL input + popover body fell outside that container and were treated as outside-clicks. MUI Popover's built-in `onClose` (already wired) handles real outside-click dismissal via its Modal backdrop. Closes baseline finding #5. | [AppToolbar.tsx](../../packages/axoview-app/src/components/AppToolbar.tsx) | manual-test-baseline #5 | low-med | (this session) |
...
| B-8 | **[x] `f38d0b4` (partial; two follow-ups deferred to user)** Split each combined `"any maskable"` icon entry in [manifest.json](../../packages/axoview-app/public/manifest.json) into two single-purpose entries so Chrome stops flagging the deprecated combined purpose. Both maskable entries reuse the existing 192/512 PNGs — not safe-zone padded for true maskable rendering, but the split removes the manifest-validation warning. `favicon.svg` 404 verified to be a stale dev-server cache (file present at `public/favicon.svg`); no manifest edit needed. Deferred to user (content creation): `form_factor: "wide"` screenshot, mobile-form-factor screenshot, properly safe-zone-padded maskable PNGs. | packages/axoview-app/public/manifest.json | B-7 smoke (2026-05-22) | low | (this session) |
...
| B-12 | **[x] `eb97586`** Toolbar Share button + share popover removed entirely (the file-explorer context-menu share-link path is the canonical share UX). Supersedes B-3 (`95a0fd5`, popover focus fix on a popover that no longer exists). favicon.svg deleted — 1MB `<svg>` wrapper around a base64 PNG that Chrome's PWA manifest validator flagged as "failed to load." index.html, manifest.json, and the AppToolbar brand mark now all point at the 96×96 PNG. `shareUrlFromUuid` + `storage.shareDiagram` retained — still used by DiagramManager + FileExplorer. Found during 2026-05-22 smoke pass on the polish trio. | AppToolbar.tsx + manifest.json + index.html + favicon.svg (deleted) | 2026-05-22 smoke | low | (this session) |
