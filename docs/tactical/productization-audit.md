...
|---|---|---|---|---|
| T1 | **C.5 — E2E rewrite (Playwright)** | New `packages/axoview-e2e/` (existing one slated for deletion per audit-locked-decision #4); new `.github/workflows/e2e-playwright.yml` to replace the Python suite; surface-anchor sweep using `data-axoview-id` per ADR 0008 Decision 5. **Gated on M9.** | locked-decision #4, ADR 0008 Decision 5 | **[~] scaffolded 2026-05-22** — tactical doc landed at [e2e-suite-rewrite.md](e2e-suite-rewrite.md) with locked scenario catalog (J1–J20 → 13 specs), POM inventory (8 surfaces), and 8-session ~270K-token budget. Session 2 (skeleton + delete existing `packages/axoview-e2e/`) is the next execution unit. |
