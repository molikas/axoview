...
| B-8 | **PWA manifest hardening.** Single file edit ([packages/axoview-app/public/manifest.json](../../packages/axoview-app/public/manifest.json)). Four Chrome console warnings observed 2026-05-22 during B-7 smoke testing: (a) no `form_factor: "wide"` screenshot — desktop install card degrades to plain; (b) no mobile-form-factor screenshot — same on mobile; (c) `favicon.svg` 404 — verify the manifest icon path against repo state (asset rename may have left a stale reference); (d) `purpose: "any maskable"` on an icon — split into separate `any` and `maskable` entries per Chrome's recommendation. | packages/axoview-app/public/manifest.json | B-7 smoke (2026-05-22) | low | bundle with B-1..B-4 cleanup wave |
