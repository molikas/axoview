...
|---|---|---|---|---|
| T1 | **C.5 — E2E rewrite (Playwright)** | New `packages/axoview-e2e/` (existing one slated for deletion per audit-locked-decision #4); new `.github/workflows/e2e-playwright.yml` to replace the Python suite; surface-anchor sweep using `data-axoview-id` per ADR 0008 Decision 5. **Gated on M9.** | locked-decision #4, ADR 0008 Decision 5 | **[~] editor-surfaces set complete 2026-05-23** — Sessions 2 + 3 + 4 + 5 shipped. **Session 2 (foundation, 5 commits):** `3ff4110` delete legacy suite, `3f087c8` skeleton + config, `62d9705` fixtures/helpers, `cce1dda` AppToolbarPOM + J1 smoke green, `516ba22` doc-sync. **Session 3 (smoke completion, 4 commits):** `ddb14d7` J20 EmptyState extension (+ EmptyStateScreenPOM + `screen-empty-import` retrofit + workers=1 pin), `f611aa2` J2 connector spec (+ `canvas-interactions` lib retrofit + `getModelConnectorCount` helper), `67571c4` J15 hotkeys spec (+ `getViewItemCount` helper), `d3d3877` doc-sync. **Session 4 (file ops, 4 commits):** `4014e59` J7+J8 JSON spec (+ ExportPopover 3 retrofits + static JSON fixture + AppToolbarPOM.clickExportJson/clickExportProjectZip), `d92aeee` J9+J10 project-ZIP spec (+ DialogsPOM + ExportProjectZipDialog confirm retrofit + programmatic JSZip fixture in helpers/projectZip.ts), `88242dc` J11+J12 icons spec (+ 4 lib-side retrofits batched into 1 rebuild + J10 wipe-and-reload stability fix), `6c902d4` doc-sync. **Session 5 (editor surfaces, 4 commits):** `d11bc8e` J3 shapes spec (CanvasPOM debut: interactionsLayer / dispatchAt / clickAt / dragFromTo / switchToRectangleMode / pressTextBoxHotkey / placeTextBoxAt + getViewRectangleCount + getViewTextBoxCount helpers; **0 lib retrofits — hotkey path**), `d0ffc3e` J4 rename spec (FileExplorerPOM debut + 3 retrofits — `dock-file-explorer-toggle` lib + `file-explorer-row` + `file-explorer-rename-input` app; MQA #14 regression test ships), `f502a62` J6 layers spec (LayersPanelPOM debut + 5 lib retrofits batched: `layers-panel-add` + `layer-row` + `layer-toggle-visibility` + `layer-toggle-lock` + `layer-item-row`; drag-drop assignment + visibility + lock; drag-lock asserts model.views[*].items[*].tile post-attempt unchanged), this commit (bookkeeping). Suite now at **22 tests / 9 spec files**, all green locally in ~5.1 min. **Twenty-three lazy `data-axoview-id` retrofits running total** — Session 2 seven + Session 3 two + Session 4 eight + **Session 5 six** (one rename, five layers). **Two new durable findings (Session 5)** — both documented inline in the e2e tactical's Session-5 actual-vs-estimate block: (1) model-level items lack a `tile` field — tile lives on view items only (placeIcon.ts); specs asserting drag semantics must read `model.views[*].items[*].tile`. (2) synthetic mouse drags MUST dispatch each MouseEvent in a separate `page.evaluate` — bundling them into one evaluate breaks drag detection because the lib's RAF-throttled scheduler only flushes once per RAF tick (useRAFThrottle.ts); without RAF ticks between dispatches, Cursor.mousemove's hasDragged comparison reads the stale snapshot and stays false. Sessions 6–7 remain (diagram-link + dialogs + share + canvas-modes; CI wiring). |
