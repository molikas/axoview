...
| B-7 | Remove nginx HTTP Basic Auth entirely (closes baseline finding follow-up to #8). Touches: `nginx.conf` (remove `auth_basic`, `auth_basic_user_file`, the `AUTH_BASIC_SETTING` placeholder, and the nested `location /api/public/` block which becomes redundant); `docker-entrypoint.sh` (remove `HTTP_AUTH_USER`/`HTTP_AUTH_PASSWORD` handling + `.htpasswd` generation + the sed-replace of `AUTH_BASIC_SETTING`); `.env.example` (drop `HTTP_AUTH_USER` + `HTTP_AUTH_PASSWORD`); `README.md` (remove `HTTP_AUTH` instructions just added in commit `8ee387a`); `docs/deployment.md` (sweep); ADR 0009 Decision 4a env-var table (remove `HTTP_AUTH` rows + note in "Files affected" that B3's nginx fix is partially reverted as part of this removal). | nginx.conf + docker-entrypoint.sh + .env.example + README.md + docs/deployment.md + ADR 0009 | Locked Decision #13 / baseline finding #8 follow-up | med | (B-7 standalone — feat! breaking change) |
