...
|---|---|---|---|---|
| T1 | **C.5 — E2E rewrite (Playwright)** | New `packages/axoview-e2e/` (existing one slated for deletion per audit-locked-decision #4); new `.github/workflows/e2e-playwright.yml` to replace the Python suite; surface-anchor sweep using `data-axoview-id` per ADR 0008 Decision 5. **Gated on M9.** | locked-decision #4, ADR 0008 Decision 5 | **[~] foundation laid 2026-05-22** — Session 2 shipped (`3ff4110` delete + `3f087c8` skeleton + `62d9705` fixtures/helpers + `cce1dda` AppToolbarPOM + J1 smoke green locally + this commit). Five lazy `data-axoview-id` retrofits landed (toolbar-save, screen-empty-create, dock-elements-toggle, dock-layers-toggle, canvas-icon-grid-item). Sessions 3–6 estimates revised +5K each (~290K total) per Session 2 actual-vs-estimate note in [e2e-suite-rewrite.md](e2e-suite-rewrite.md). Session 3 (smoke completion — J20 + connector + hotkeys) is the next execution unit. |
