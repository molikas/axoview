...
| **M10** | Productization ship gate — **[x] met 2026-05-23** | Tagged release; container image published; Cloudflare deploy verified end-to-end in session mode (signup-free, public URL, multi-user session isolation honoured); README + deployment.md reflect the shipped reality. **v1.0.0 shipped 2026-05-23.** CI all green on master push; CHANGELOG bump commit `1131a32`; GitHub Release published; axoview.pages.dev deploy live and manually smoked. (T4 external dashboard actions — branch protection + repo metadata — remain as out-of-repo follow-ups; CodeQL repo-toggle already done.) | M6, M7, M8, M9 |
...
| 11 | **`axoview-lib` stays monorepo-only; no npm publish.** Docker + Cloudflare are the productization surface. Lib publishing posture locked 2026-05-20 by user decision; closes A.8 #A9 with revert of the pre-Phase-C bundle's publish wiring (`@semantic-release/npm` removed; `axoview-lib/package.json` set to `private: true`). Future re-publish decision requires explicit reversal of this row. | The Docker container + CDN-deployed SPA cover the consumer story without npm-distribution overhead. Maintaining a parallel npm release path adds CI surface, registry-token management, and versioning ceremony for no current user need. |
| 12 | **Docker Hub publish deferred to a future feature** (own ADR + tactical when wanted). Day-1 deploy path = clone the repo + `docker compose up --build`. No tech debt left in tree: `.github/workflows/docker.yml` deleted, `compose.yml` builds locally (`build: .` replaces `image: molikas/axoview:latest`), T2 G6 (container image scanning) and the `DOCKERHUB_USERNAME` / `DOCKERHUB_TOKEN` external-action items removed. Locked 2026-05-21 by user decision (baseline finding #8 / B-6). | A published image is a product surface — versioning policy, security-scan gating, breaking-change comms, registry credentials all need a deliberate first-class design pass. Shipping it half-built creates trust debt; deferring keeps the integration branch clean and makes the eventual feature easier to scope. |
| 13 | **nginx HTTP Basic Auth removed.** It was a FossFLOW-era simple-password layer overlapping with the ADR-0009-D4 `AUTH_MODE` contract. Two auth layers caused confusion + forced B3-style carve-outs. Day-1 deploy = no auth (shared instance); deployers wanting auth use `AUTH_MODE=shared-token` or `AUTH_MODE=cf-access` per ADR 0009 D4. Future Basic-Auth-style password layer (if ever wanted) is a separate feature with own ADR + tactical. Locked 2026-05-21 by user decision; closes baseline finding #8 follow-up. | One auth contract is comprehensible; two are not. Basic Auth duplicated `AUTH_MODE`'s purpose at a different layer (nginx vs Express/Worker) and forced B3-style nested-location carve-outs to keep share routes reachable. Removing it collapses the surface to a single ADR-governed knob and eliminates the carve-out as collateral. |
| 14 | **GitHub Pages workflow removed; Cloudflare Pages native git integration is the canonical deploy mechanism.** `.github/workflows/pages.yml` deleted; `.github/workflows/release.yml` `workflow_run` trigger rewired from `"Deploy static content to Pages"` → `"Run Tests"`. T2 G5 (add Cloudflare Pages deploy automation via GH Actions) dropped from the tactical entirely. A.8 #A5 finding ("Cloudflare deploy automation MISSING") is **superseded by reality** — the audit missed that CF's native git integration was already deploying `axoview.pages.dev` on every master push. No GH-Actions-mediated CF deploy workflow added; future need is a separate feature with its own ADR. Locked 2026-05-22 by user decision; closes A.8 #A5. | Two deploy paths (GitHub Pages + Cloudflare native) created split-brain about which URL is canonical; the GH Pages workflow built into `PUBLIC_URL: /Axoview/` artifacts that the canonical `axoview.pages.dev` deploy doesn't use. Deleting it collapses the deploy story to one path (CF native) and removes the dead `workflow_run` link that gated Release on a stale chain. |
| 15 | **Error UX contract codified as ADR 0011 (Accepted 2026-05-22).** All failure-of-intent paths surface explicit error Dialogs; silent redirects + notification-only fallbacks for intent failures are anti-pattern. Existing dialogs (LocalModeShareErrorDialog, ReadonlyLoadErrorDialog) conform without retrofit drift. New `PublicShareLoadErrorDialog` lands per ADR 0011 for the B-9a S1 case. Future error surfaces follow per-scenario `<Scenario>ErrorDialog.tsx` naming + `dialog.<scenario>.*` i18n namespace. Carve-outs preserved: in-dialog inline error states + side-effect notification toasts + boot-time fallback per ADR 0009 D2 + form-field validation. Locked 2026-05-22 by user decision (path α from B-9a checkpoint). | Two dialogs converged independently on the same shape; codifying the latent contract removes ambiguity for B-9b retrofit + future error surfaces. The 20-surface silent-failure inventory now has a defined retrofit target rather than unstructured technical debt. |
| 16 | **T2 (git-automation hardening) wraps 2026-05-22.** CI gates active on every push/PR: ESLint hard-fail (G1), coverage threshold hard-fail (G2), build-output shape (`_routes.json` + `_headers`) hard-fail (G8), commitlint local hook (G3), CodeQL static analysis (G4), Worker bundle-size budget < 1 MB uncompressed per ADR 0009 D8 (G7), continuous knip soft-fail (G10). G5 + G6 dropped per Locked Decisions #12 + #14. Promotion of knip to hard-fail and the CodeQL repo-toggle activation are **external T4 actions**, not in-repo work. **M8 milestone met for in-repo scope.** | Closes the M8 productization gate's CI-baseline + security-scan + commit-hygiene halves at once. Bundling the final three rows (G4 + G7 + G10) as a wave keeps the bookkeeping atomic and the closure marker single-dated. The two external follow-ups (CodeQL toggle, knip promotion) are recorded explicitly so M8's "in-repo" framing doesn't paper over outstanding human action. |
| 17 | **M10 ship gate met 2026-05-23 (v1.0.0). Audit substantively complete.** Sequencing for closeout: (a) README FossFLOW sweep + semantic-release successComment disable (this commit's cleanup session); (b) 3-session technical review artifact at `docs/technical-review-2026-05.md` (snapshot for independent reviewer; comprehensive every-git-tracked-file inventory + Mermaid sequence diagrams + dual-lens reviewer prompts); (c) T4 user external actions (branch protection + repo metadata; CodeQL repo-toggle already done); (d) `/feature` wrap productization-audit ceremony. Audit doc itself stays present until step (d). | Productization is now end-to-end (tagged release + live CF deploy + smoked). The remaining work is documentation (review artifact) + external dashboard (T4) + ceremony (wrap), none of which gate further code change. Sequencing this explicitly prevents the "is it done?" ambiguity that a half-closed audit would carry — every step has a named output and a known successor. |
...
|---|---|---|---|
| **N1** | **The largest A.2 deliverable is a pre-rename report that needs re-running.** ~70% of its "Unused files" rows are stale (resolved by deletion clusters that ran after the report). A fresh knip run would be more actionable than this audit's classification of the old report. **Resolved 2026-05-22 (commit `32c43b8`, T2 G10):** continuous knip wired into `test.yml` (`npx knip --reporter compact`, soft-fail via `continue-on-error: true`). Fresh report runs on every push/PR; promotion to hard-fail gated on one full week of green master per git-automation-hardening tactical Locked Decision 7. | reports/knip.txt + A.2.2 | C.8 git-automation tactical: schedule a weekly `npx knip > reports/knip.txt` job. The output is gitignored but useful as a session artifact. |
...
| ~~E2E Tests~~ | ~~`.github/workflows/e2e-tests.yml`~~ | n/a | n/a | **Deleted 2026-05-23 per C.2 I9 (commit `f45e677`)** — replaced by `e2e-playwright.yml` below. |
| **E2E (Playwright)** | [.github/workflows/e2e-playwright.yml](../../.github/workflows/e2e-playwright.yml) (67 lines, added 2026-05-23 commit `213b492`) | `pull_request` to master + integration, `push` to master, `workflow_dispatch` | Node 22 → `npm ci` → `npm run build:lib` → `npx playwright install --with-deps chromium` → `npm run test:e2e:ci` (Shape A: webServer block in playwright.config.ts spawns dev server). Draft PRs skipped; per-ref concurrency cancels superseded runs; 30-min job cap; uploads `playwright-report` + `test-results` on failure. | **healthy (pending first PR trigger).** |
...
| ~~Deploy static content to Pages~~ | ~~`.github/workflows/pages.yml`~~ | n/a | n/a | **Deleted 2026-05-22 per Locked Decision #14** — Cloudflare Pages native git integration is the canonical deploy. Commit `71a4b32`. |
| **Release** | [.github/workflows/release.yml](../../.github/workflows/release.yml) (42 lines) | `workflow_run: Run Tests` (success only — rewired 2026-05-22 per Locked Decision #14) | Node 22 → `npx semantic-release` → publishes to GitHub Releases + tags (npm publish dropped 2026-05-20 per Locked Decision #11) | **healthy** |
...
**Workflow chain visualization (current state, post T1 Session 7):**

```
   push to master ──┬──> Run Tests ──> Release (workflow_run success)
   PR to master/    │
   integration  ────┼──> Run Tests
                    │
                    └──> E2E (Playwright) (independent — does NOT chain off Run Tests)
                            │
                            └─> Dependabot Auto-Merge (PR-only, dependabot-only)

   push to master ──> CodeQL (T2 G4)
   PR to master ────> CodeQL
   weekly         ──> CodeQL
```

**Chain mutations recorded in this audit:**
- `docker.yml` deleted 2026-05-21 per Locked Decision #12 (Docker Hub publish deferred).
- `pages.yml` deleted 2026-05-22 per Locked Decision #14 (CF Pages native git integration is canonical).
- `ethicalcheck.yml` deleted 2026-05-20 per A.8 #A3 (upstream template, unrelated demo API).
- `release.yml` `workflow_run` trigger rewired from `"Deploy static content to Pages"` → `"Run Tests"` 2026-05-22 with the `pages.yml` deletion. Targets Run Tests not E2E Tests because the latter is the legacy Python/Selenium suite slated for deletion per Locked Decision #4 + C.2 I9.
- `e2e-tests.yml` deleted + `e2e-playwright.yml` added 2026-05-23 per C.2 I9 + T1 Session 7. The new E2E workflow runs independently of Run Tests (not chained via `workflow_run`); release.yml remains pinned to `"Run Tests"` and is NOT gated on E2E green.
...
| # | Observation | Evidence | Suggested follow-up |
|---|---|---|---|
| **A1** | **ESLint never runs in CI.** Single biggest gap in A.8.2. The repo has a flat-config `eslint.config.mjs` at root; no workflow invokes it. Lint errors in `npm run lint` only run for lib (which uses `tsc --noEmit` under the `lint` name, confusingly). **Resolved 2026-05-22 (commit `4d74093`):** `npx eslint .` added as a hard-fail step in `test.yml` between `npm ci` and tests. Lint baseline at the time of wiring: 0 errors, 196 warnings (warnings stay informational; no `--max-warnings` flag). | A.8.2 lint row + [test.yml](../../.github/workflows/test.yml) | C.8 git-automation tactical adds an `eslint` step to `test.yml` (or a dedicated `lint.yml`). |
| **A2** | **Coverage threshold gate is defeated by `|| npm test` fallback.** Even with lib's `coverageThreshold: 10`, the CI command `npm test -- --coverage || npm test` swallows the threshold failure and re-runs tests without coverage. Result: coverage **never fails CI.** **Resolved 2026-05-22 (commit `967b2c7`):** `|| npm test` fallback removed from `test.yml`; threshold failures now propagate. | [test.yml:31](../../.github/workflows/test.yml#L31) | Remove the `|| npm test` fallback; let threshold failures be real failures. Single-line fix. |
...
| **A4** | **`e2e-tests.yml.backup` is checked into the workflows directory.** Files with `.yml.backup` suffix are ignored by Actions, but leaving them in `.github/workflows/` is a maintenance smell — future editors may confuse them with active workflows. **Resolved 2026-05-20 (commit `264887a`, C.2 Q8):** file deleted. | [.github/workflows/e2e-tests.yml.backup](../../.github/workflows/e2e-tests.yml.backup) | C.2 row. Single-file deletion. |
| **A5** | **Cloudflare Pages deploy is manual.** The `pages.yml` workflow targets GitHub Pages (with `PUBLIC_URL: /Axoview/`). M10 (productization ship gate) requires a CF Pages deploy triggered on master push. **This is a productization blocker.** **Resolved 2026-05-22 by reality (commit `71a4b32`):** native CF git integration handles master-push deploys; no GH Actions workflow needed. The audit missed that the integration was already live. `pages.yml` deleted; `release.yml` rewired off the stale Pages chain. T2 G5 dropped per Locked Decision #14. | [pages.yml](../../.github/workflows/pages.yml) (deleted) | C.8 git-automation tactical adds a `cloudflare-pages.yml` workflow that uses `cloudflare/pages-action` (or `wrangler pages deploy`) on master push. **Superseded 2026-05-22 by Locked Decision #14** — no GH-Actions-mediated CF deploy added; native integration is canonical. |
...
| **A7** | **No commitlint means semantic-release silently skips malformed commits.** A subject typed `feature:` instead of `feat:` produces no release. **Risk increases with multiple committers.** **Resolved 2026-05-22 (commit `8574fca`):** `@commitlint/cli` + `@commitlint/config-conventional` + `simple-git-hooks` wired at repo root; `commit-msg` hook runs commitlint against the conventional preset; `prepare` script auto-installs the hook on `npm install`. | absence | C.2 row: add `commitlint` + simple-git-hooks (or husky if preferred). Low-effort. |
| **A8** | **CodeQL absence is the single biggest security-scanning gap.** Free for public repos, no operational cost. Catches the most common JS/TS security smells (XSS, prototype pollution, regex DoS, etc.). **Resolved 2026-05-22 (commit `228bb5f`):** `.github/workflows/codeql.yml` added — push-to-master + PR-to-master + weekly cron triggers; single `javascript-typescript` matrix entry; results land on Security tab once the repo-level CodeQL toggle is enabled (T4 external-action checklist item). | A.8.4 | C.8 row: add `codeql.yml` (GitHub provides a default template). |
| **A9** | **npm publish for `axoview-lib` is unverified.** `.releaserc.json` includes the github + git plugins but **no `@semantic-release/npm` plugin entry.** The lib has `private: false` and a `repository`, suggesting publish intent. The first release after the rename may have failed to publish to npm, OR the publish flow uses a different mechanism not documented here. | [.releaserc.json](../../.releaserc.json) | Verify with `npm view axoview` — has the lib been published since the rename? If not, M8 cannot ship until npm-publish wiring is added. **Resolved 2026-05-20:** lib publishing posture LOCKED as monorepo-only (user decision; rationale: Docker container + Cloudflare deploy is the product surface). `@semantic-release/npm` removed from `.releaserc.json`; `axoview-lib/package.json` set to `private: true`. Rename Phase 10's "npm publish" item closes with this decision. |
...
| C.2 — Cleanup plan | **drafted 2026-05-20; Section 1 complete 2026-05-20** (Bundle A `264887a` Q1/Q3/Q4/Q6/Q7/Q8; Bundle B `39a44c1` Q2/Q5/Q9). **Section 2 complete 2026-05-20** (B1 `e307c68`; B2 `cff3942`; B3 `e0ee6f6`). **Section 3 Wave 3a complete 2026-05-20** (Bundle 3a-A `50146d2` I4/I5/I6/I7; Bundle 3a-B `6e1b28c` I2/I3). **Wave 3b I1 complete 2026-05-20** (`0810690` — dual-probe collapse + `/api/storage/status` deletion). I8 held per release-window gate; I9 deferred to T1 (E2E rewrite). Section 3 substantively complete — only I8 + I9 remain, both deferred-by-gate. **Section 5 progress 2026-05-22:** B-1 `2a04061` + B-2 `84f4e09` (diagram-link UX) added to the already-shipped B-5 `315f395`, B-6 `8ee387a`, B-7 `4bc0005`. B-9a Error UX contract shipped (`f843137` + `1bacd0b` + `072e924` + bookkeeping). **Polish trio + smoke follow-ups shipped this session: B-3 `95a0fd5` (share popover focus — later superseded by B-12 toolbar Share removal), B-4 `0c9a1f9` (HelpDialog completeness), B-8 `f38d0b4` (PWA manifest split; favicon.svg loading warning resolved by B-12), B-11 `681c7a6` (preview→back popup), B-12 `eb97586` (drop toolbar Share + delete 1MB favicon.svg).** B-9b queued for T1-gated execution (enumerated S2–S20 working catalogue). B-10 filed this session (knip soft-fail triage), queued for next polish wave. **2026-05-23 closeout disposition:** every remaining row is **deferred-by-design** for post-audit polish; nothing else gates M10 / audit wrap. B-9b — T1-gated → T1 now complete (Session 8, 2026-05-23) → re-classified as polish backlog post-audit (no longer blocked). B-10 — filed-not-executed; knip soft-fail triage moves to polish backlog. B-14 — filed-not-executed (properties-panel redundant "X"); polish backlog. I8 — release-window gated, auto-fires on the first release after 2026-06-01. | [C.2 section above](#c2-cleanup-plan-drafted-2026-05-20) — 4-section spine (9 quick-wins · 3 bugs · 9 ADR-implementation rows · 4 spawned tacticals) + sequencing recommendation |
...
| C.5 — E2E rewrite tactical | **complete 2026-05-23 — M9 met** (13 spec files / 33 tests green locally + on CI). Sessions 2–8 shipped per the e2e tactical's session ledger; Session 8 closed the three CI-only drag failures via two POM fixes (`307a8d7` CanvasPOM RAF-tick, `2232860` DialogsPOM dialog-detach). Final CI run `26335821105` 33/33 in ~3.0 min. | [e2e-suite-rewrite.md](e2e-suite-rewrite.md) — preserved through `/feature wrap` (PLAN.md Phase 2D entry + tactical deletion pending). |
...
| C.8 — Git-automation tactical | **complete 2026-05-22** — G9 (`8ee387a`) + G2 (`967b2c7`) + G1 (`4d74093`) + G3 (`8574fca`) + G8 (`72f12de`) + G4 (`228bb5f`) + G7 (`01286f8`) + G10 (`32c43b8`) all shipped; G5 dropped per Locked Decision #14; G6 dropped per Locked Decision #12. CI baseline now enforces ESLint, coverage threshold, build-output shape, commitlint locally, CodeQL static analysis, Worker bundle size, and continuous knip (soft-fail). | [git-automation-hardening.md](git-automation-hardening.md) — see T2 row in C.2 Section 4 for the per-row status |
| C.9 — Workflow doc + skill alignment | **completed 2026-05-20** | [docs/workflow.md](../workflow.md), 4 skill body edits |
| C.10 — Error UX contract ADR 0011 | **Accepted 2026-05-22** | [ADR 0011](../adr/0011-error-ux-contract.md) — extracted from the two converged dialog precedents (LocalModeShareErrorDialog, ReadonlyLoadErrorDialog). 20-surface silent-failure inventory in B-9a. B-9a S1 applied (`PublicShareLoadErrorDialog` for public-share load failure); S2–S20 enumerated as B-9b working catalogue, gated on T1 |
...
**M10 readiness (2026-05-23):** with M9 closed (T1 Session 8), the M10 productization ship gate is one external-action set away. The remaining gate is row T4 — "GitHub-dashboard checklist (out-of-repo)" — branch protection on `master` (require PR, require status checks, no force push, signed-commits optional) + repo Description / Topics / Homepage URL set. CodeQL repo-toggle is already done (it was the prior remaining T4 sub-item; verified live in the Security tab post-2026-05-22). After T4 lands, the merge of the open audit PR → master fires the Release workflow + Cloudflare auto-deploy; manual smoke against axoview.pages.dev (per the PR steps helper) is the final M10 confirmation step.
...
### Section 1 — Quick wins (no risk gating; ship individually) — **complete 2026-05-20**
...
| Q2 | **[x] `39a44c1`** Close G1 — rewrote [`packages/axoview-lib/LICENSE`](../../packages/axoview-lib/LICENSE): replaced `Copyright (c) 2025 Mark Mankarious` with `Copyright (c) 2026 Igor Sidenica — Axoview project`; MIT permission body verbatim from root LICENSE; separated attribution paragraph below names Mark/Isoflow lineage. | `packages/axoview-lib/LICENSE` | A.7 row 12 + G1 (partial) | low | Q2 |
...
| Q5 | **[x] `39a44c1`** Deleted `.github/FUNDING.yml` (locked 2026-05-20). Axoview does not accept funding; no README callout. Closes G2. | `.github/FUNDING.yml` (deleted) | A.7 row 10 + G2 | low | Q5 |
...
| Q9 | **[x] `39a44c1`** Deleted legacy `.md` pair (locked 2026-05-20) — `bug-report.md` + `feature-request.md`. YAML form (`bug_report.yml` + `feature_request.yml`) is canonical. Closes G3. | `.github/ISSUE_TEMPLATE/*.md` (deleted) | A.7 row 5 + G3 | low | Q9 |

### Section 2 — Real shipping bugs (prioritized)

| # | Action | Surface | Driving finding | Risk | Bundle |
|---|---|---|---|---|---|
| B1 | **[x] `e307c68`** Renamed `SessionModeBanner` → `LocalModeBanner` (per ADR 0008 Decision 1). One file rename + import update in [`packages/axoview-app/src/App.tsx`](../../packages/axoview-app/src/App.tsx). Semantic-correctness fix — the banner only fires in LOCAL mode despite its name. Closes A.4 #C2. | [packages/axoview-app/src/components/LocalModeBanner.tsx](../../packages/axoview-app/src/components/LocalModeBanner.tsx), App.tsx | A.4 #C2 | low | B1 |
| B2 | **[x] `cff3942`** Implemented `LocalModeShareErrorDialog` per ADR 0009 Decision 3. Triggered by `isReadonlyUrl && !serverStorageAvailable`; explicit error replaces the prior silent empty-diagram + generic failure-toast. Dismiss navigates back to `/` so the app boots Local mode normally. Also short-circuits the share-fetch in `DiagramLifecycleProvider` when `!serverStorageAvailable` so the generic toast no longer fires. i18n strings added to `en-US` (other locales fall back via `t(key, defaultEnglish)`). Closes A.4 #C5. | [LocalModeShareErrorDialog.tsx](../../packages/axoview-app/src/components/LocalModeShareErrorDialog.tsx), [App.tsx](../../packages/axoview-app/src/App.tsx), [DiagramLifecycleProvider.tsx](../../packages/axoview-app/src/providers/DiagramLifecycleProvider.tsx), en-US locales | A.4 #C5, ADR 0009 Decision 3 | med | B2 |
| B3 | **[x] `e0ee6f6`** Added nested `location /api/public/ { auth_basic off; ... }` block to [`nginx.conf`](../../nginx.conf). Self-host share viewers can now read public-namespace diagrams without the basic-auth challenge — restoring parity with the Express middleware carve-out (server.js). Closes A.6 #D3. | [nginx.conf](../../nginx.conf) | A.6 #D3, ADR 0010 Decision 4 | med | B3 |

### Section 3 — ADR-implementation rows (gated on ADR acceptance)

| # | Action | Surface | Gate | Risk | Bundle |
|---|---|---|---|---|---|
| I1 | **[x] `0810690`** Collapsed dual-probe + deleted `/api/storage/status` from Express ([routes.js](../../packages/axoview-backend/src/routes.js), [server.js](../../packages/axoview-backend/server.js)) and Worker ([app.ts](../../packages/axoview-worker/src/app.ts), [auth.ts](../../packages/axoview-worker/src/auth.ts)). Boot path now issues a single `GET /api/config`; `serverStorage` flag is the canonical mode signal. `LocalStorageProvider.usingServer` is set externally by `AppStorageContext` via the new `StorageManager.setServerStorage(boolean)` (`initialize()` removed). `fetchRuntimeConfig` swallow-path replaced with explicit `console.warn` per ADR 0009 D2. The `RuntimeConfig.serverStorage` *type* field stays — it transitions from dead (A.4 #C1) to live (canonical consumer = `AppStorageContext`); removing the typed field while keeping the wire field would create a two-sources-of-truth problem. Tests rewritten: single-probe contract for `AppStorageContext`; `usingServer` injected directly in `LocalStorageProvider`. **BREAKING CHANGE** — deployers polling `/api/storage/status` must migrate to `/healthz` (Wave 3a / ADR 0010 D8). Closes A.4 #C1+C4, A.6 #D1+D2, Theme 1. | mode-detection (4 files) | ADR 0009 Decision 2 (Accepted) | med | I1 |
| I2 | **[x] `6e1b28c`** fs adapter atomicity — `put()` now uses tmp-file + rename per ADR 0010 Decision 3. Per-pid tmp filename + unlink-on-failure cleanup. Backend has no jest runner yet (gap in C.8); verification path is Docker smoke. | [fs.js](../../packages/axoview-backend/src/adapters/fs.js) | ADR 0010 Decision 3 (Accepted) | med | I2 |
| I3 | **[x] `6e1b28c`** `/healthz` endpoint + Dockerfile HEALTHCHECK + compose healthcheck per ADR 0010 Decision 8. Express route returns `{ ok, adapter, storage_writable }` (200) or `{ ok: false, reason }` (503); cached 10s; no auth (path is outside the `/api/` middleware scope). Dockerfile uses `wget` (already in `node:22-alpine`). `healthcheck:` block added to both `compose.yml` + `compose.dev.yml`. Closes the A.6.2 HEALTHCHECK gap. | server.js + Dockerfile + compose.yml + compose.dev.yml | ADR 0010 Decision 8 (Accepted) | med | I3 |
| I4 | **[x] `50146d2`** Worker-package wrangler.toml drift callout added (top-of-file comment) per ADR 0009 Decision 5. | [packages/axoview-worker/wrangler.toml](../../packages/axoview-worker/wrangler.toml) | ADR 0009 Decision 5 (Accepted, post-review downgrade) | low | I4 |
| I5 | **[x] `50146d2`** Three component file renames per ADR 0008 Decision 1: `components/ExportDialog.tsx` → `ExportSingleDiagramDialog.tsx`; `components/fileExplorer/ExportDialog.tsx` → `ExportProjectZipDialog.tsx`; `StorageManager.tsx` (modal) → `LocalStorageInspector.tsx`. The `StorageManager` orchestrator class in `services/storage/` is preserved per the explicit ADR 0008 D1 carve-out. Component exports + 3 import-site files updated. | 3 component files + 3 import sites | ADR 0008 Decision 1 (Proposed; pending acceptance) | low | I5 |
| I6 | **[x] `50146d2`** Deleted `packages/axoview-lib/src/components/Sidebars/LeftSidebar.tsx` (A.3 #3 pure orphan). `docs/architecture.md` entry removed in same commit per workflow design principle 5. | [packages/axoview-lib/src/components/Sidebars/LeftSidebar.tsx](../../packages/axoview-lib/src/components/Sidebars/LeftSidebar.tsx) | A.3 #3 | low | I6 |
| I7 | **[x] `50146d2`** Deleted `packages/axoview-app/src/paymentFlowExample.json` (A.6.7 row 15; zero importers verified). | `packages/axoview-app/src/paymentFlowExample.json` | A.6.7 row 15 | low | I7 |
| I8 | **`__fossflow__` deprecation alias + `migrationShim.ts` removal** — single deletion in [`packages/axoview-lib/src/Axoview.tsx:143-162`](../../packages/axoview-lib/src/Axoview.tsx#L143) + [`migrationShim.ts`](../../packages/axoview-app/src/utils/migrationShim.ts) + its test. **Gate (re-locked 2026-05-20): next release after 2026-06-01.** Hold-pending-release-window framing stays; trigger changed from "first axoview-lib npm publish" to a calendar-window after lib was locked as monorepo-only (Locked Decision #11). | Axoview.tsx + migrationShim | A.3 #10, #13 | low | I8 |
| I9 | **[x] `213b492` + `f45e677` (2026-05-23, T1 Session 7)** Deleted legacy `e2e-tests/` (9 pytest files, requirements.txt, run-tests.sh, test-base-paths.sh, test-diagram.json, Cargo.lock leftover, README/SETUP) + `.github/workflows/e2e-tests.yml` Selenium-Chrome driver. New `.github/workflows/e2e-playwright.yml` landed first (Shape A: webServer block in `playwright.config.ts` spawns `npm run dev`, no explicit serve / wait-on YAML lifecycle); deletion landed second per locked-decision #4 "no window where both suites are absent." Stale-ref sweep: `.gitignore` (`/e2e-tests/target` removed; `test-results/` + `playwright-report/` added), `.dockerignore` (`e2e-tests` line removed), `docs/testing.md` (E2E paragraph rewritten to point at packages/axoview-e2e/). Historical refs in this audit doc + e2e-suite-rewrite.md + git-automation-hardening.md preserved per project convention. Closes locked-decision #4 + C.5 I9 bundle. | `e2e-tests/` directory + `.github/workflows/e2e-tests.yml` (deleted) | locked-decision #4 + C.5 | high | I9 + C.5 |

### Section 4 — Spawned tacticals (separate work units)

| # | Spawned tactical | Scope summary | Driving finding | Status |
|---|---|---|---|---|
| T1 | **C.5 — E2E rewrite (Playwright)** | New `packages/axoview-e2e/` (existing one slated for deletion per audit-locked-decision #4); new `.github/workflows/e2e-playwright.yml` to replace the Python suite; surface-anchor sweep using `data-axoview-id` per ADR 0008 Decision 5. **Gated on M9.** | locked-decision #4, ADR 0008 Decision 5 | **[x] M9 met 2026-05-23.** All 13 spec files / 33 tests green locally + on CI (final run `26335821105`, ~3.0 min). Session 8 (`0c5f7dc` instrumentation, `307a8d7` CanvasPOM.dispatchAt RAF-tick fix for synthetic in-canvas drags, `2232860` DialogsPOM dialog-detach fix for native panel→canvas drags through MUI dialogs, `21029bf` instrumentation removal, this commit doc-sync) closed the three CI-only drag failures (layers J6 lock + icons J11 + icons J12). Two durable findings — both NOT in the session's three pre-set hypotheses (DPI/viewport, RAF-per-event, pointer-event semantics) — documented in the e2e tactical's Session 8 actual-vs-estimate block. Earlier ledger: **Session 2 (foundation, 5 commits):** `3ff4110` delete legacy suite, `3f087c8` skeleton + config, `62d9705` fixtures/helpers, `cce1dda` AppToolbarPOM + J1 smoke green, `516ba22` doc-sync. **Session 3 (smoke completion, 4 commits):** `ddb14d7` J20 EmptyState (+ EmptyStateScreenPOM + `screen-empty-import` + workers=1 pin), `f611aa2` J2 connector (+ `canvas-interactions` retrofit + `getModelConnectorCount`), `67571c4` J15 hotkeys (+ `getViewItemCount`), `d3d3877` doc-sync. **Session 4 (file ops, 4 commits):** `4014e59` J7+J8 JSON, `d92aeee` J9+J10 project-ZIP, `88242dc` J11+J12 icons, `6c902d4` doc-sync. **Session 5 (editor surfaces, 4 commits):** `d11bc8e` J3 shapes (CanvasPOM debut, 0 retrofits), `d0ffc3e` J4 rename (FileExplorerPOM debut, 3 retrofits), `f502a62` J6 layers (LayersPanelPOM debut, 5 retrofits), `c4ff272` doc-sync. **Session 6 (diagram-link + dialogs + share + canvas-modes, 5 commits):** `d0cfd78` J5 multi-diagram (NodeInfoTabPOM debut + 5 retrofits; bridge `ui.setState` pattern for readonly NodePanel mounting), `943d10c` J16/J17/J18 dialogs (SettingsDialogPOM + HelpDialogPOM debut + 10 retrofits batched), `ad2a6b2` J13/J14 share (J14 multi-context mocked-backend round-trip via page.route + clipboard permissions + 7 app retrofits), `e5927e6` J19 canvas-modes (CanvasPOM extension + 1 lib retrofit via new optional `IconButton.dataAxoviewId` pass-through prop), this commit (bookkeeping). Suite now at **33 tests / 13 spec files**, all green locally in ~5.3 min. **Thirty-six lazy `data-axoview-id` retrofits running total** — Sessions 2–5 twenty-three + **Session 6 thirteen** (5 multi-diagram + 10 dialogs + 7 share + 1 canvas-modes). **Three new durable findings (Session 6)** — all documented in the e2e tactical's Session-6 actual-vs-estimate block: (1) lib `IconButton` wrapper does NOT pass through arbitrary props; Session 6 added an optional `dataAxoviewId` prop forwarded to the underlying `<Button>` (existing call sites untouched, future `canvas-tool-*` retrofits inherit the opt-in). (2) Zustand action calls across `page.evaluate` return silently; direct `ui.setState({...})` writes through the API React subscribers bind to (J5.3 readonly NodePanel pattern). (3) Mocked-backend share flows need per-context `page.route()` installation — `browser.newContext()` creates an isolated request pipeline; J14 installs the same mocks on both contexts separately. Bonus carry-forward: MUI `Dialog`/`Popover` `PaperProps` typing doesn't expose `data-*` but the runtime forwards them — stamp via `PaperProps={{ ..., 'data-axoview-id': '...' } as any}`. **Session 7 (CI wiring + I9 closure, 3 commits):** `213b492` add `.github/workflows/e2e-playwright.yml` (Shape A: webServer block in `playwright.config.ts` spawns `npm run dev`, no explicit YAML serve / wait-on lifecycle), `f45e677` delete legacy `e2e-tests/` Python/Selenium suite + `.github/workflows/e2e-tests.yml` (per audit row I9 + locked-decision #4 "no window where both suites are absent"), `f97c0ce` doc-sync. CI first-trigger pending until first PR — Session 8 then closed the three drag failures surfaced on that first CI run. Earlier session findings preserved in the tactical doc's per-session blocks. |
| T2 | **C.8 — Git automation hardening** | Add ESLint to CI (closes A.8 #A1); remove `\|\| npm test` coverage fallback (closes A.8 #A2); add `commitlint` + simple-git-hooks (closes A.8 #A7); add `codeql.yml` (closes A.8 #A8 + #A4); ~~add Cloudflare Pages deploy automation~~ (G5 dropped 2026-05-22 per Locked Decision #14); add container scanning (G6 dropped 2026-05-21 per Locked Decision #12); add CI bundle-size check for Worker per ADR 0009 Decision 8; add CI verification that `_routes.json` + `_headers` are emitted by build per ADR 0009 Decision 5; add continuous knip in CI (closes A.2 #N1). **Gated on Phase C ADRs accepted (satisfied).** | A.8 #A1, #A2, #A4, #A5, #A7, #A8, A.2 #N1, ADR 0009 | **complete 2026-05-22** — G9 `8ee387a` (2026-05-21) + G2 `967b2c7` + G1 `4d74093` + G3 `8574fca` + G8 `72f12de` + G4 `228bb5f` + G7 `01286f8` + G10 `32c43b8` all shipped 2026-05-22; G5 dropped per Locked Decision #14; G6 dropped per Locked Decision #12. See Locked Decision #16 for the wrap summary and [git-automation-hardening.md](git-automation-hardening.md) for the row-level ledger. |
...
Rows seeded from the [manual-test-baseline](../manual-test-baseline.md) walk (Igor, 2026-05-21). All dispositions are **bug**; numbering is local (B-1..B-12) to avoid colliding with prior Q/I/B/T sequences. B-5 + B-6 executed first session; B-1 + B-2 + B-7 followed; B-3 + B-4 + B-8 landed this polish session; B-9a shipped 2026-05-22; B-9b queued behind T1; B-10 filed 2026-05-22 from T2 G10 (knip soft-fail baseline) and queued for next polish wave; B-11 + B-12 added 2026-05-22 from smoke pass on the polish trio (readonly load popup + toolbar Share / favicon.svg cleanup); B-13 filed 2026-05-22 from i18n investigation (checkpoint fired — public/i18n/app/ is partially live, deletion would break de-DE + id-ID).

| # | Action | Surface | Driving finding | Risk | Bundle |
|---|---|---|---|---|---|
| B-1 | **[x] `2a04061` + follow-up** Option B (user blessed 2026-05-22). NodeInfoTab gains an "open linked diagram" `IconButton` next to the Autocomplete (visible when `modelItem.link` is set); NodePanel readonly's linked-diagram link uses the same path. Both surfaces dispatch an `axoview-navigate-to-diagram` `CustomEvent` (the lib's existing decoupling pattern, see PANEL_EVENT in NodePanel.tsx:22); App.tsx listens and calls React Router `navigate(\`/display/${id}\`)` for same-window SPA navigation. `<a href>` is preserved so Ctrl/Cmd/Shift/middle-click still triggers browser-native open-in-new-tab. `handlePreviewClick` switched from `window.open` to same-window `navigate`; the toolbar Preview button's `disabled` gate relaxed from `!serverStorageAvailable \|\| !currentDiagramId` to just `!currentDiagramId` so Local mode users can preview localStorage-backed diagrams. (Smoke 2026-05-22: new-tab approach failed in Session mode because the second tab could not carry session/auth state — same-window navigation is the robust answer.) DiagramLifecycleProvider's owner-readonly load path gains parity with the public-share path (`iconPackManager.loadPacksForDiagram`, `defaultColors` merge, `frozenInitialDataRef` fallback). The `window.location.href = '/'` catch-handler cascade is replaced with `setReadonlyLoadFailed(true)` + a new `ReadonlyLoadErrorDialog` (mirrors the B2 `LocalModeShareErrorDialog` pattern from `cff3942`). `showLocalModeShareError` narrows from `isReadonlyUrl && !serverStorageAvailable` to `isPublicShareUrl && !serverStorageAvailable` — Local mode can now serve its own `/display/<diagramId>` view against `localStorage`. ADR 0009 Decision 3 gains a dated addendum codifying the share-route vs owner-readonly distinction. Build + 93/93 jest suites / 1009 passes at both commits. Investigation findings preserved below the table for audit lineage. Closes baseline findings #1, #3, #4. | NodeInfoTab.tsx + NodePanel.tsx + DiagramLifecycleProvider.tsx + App.tsx + AppToolbar.tsx + ReadonlyLoadErrorDialog (new) + en-US.json + ADR 0009 | manual-test-baseline #1, #3, #4 | med | B-1 |
| B-2 | **[x] `84f4e09`** Filter the current diagram out of the `linkedDiagrams` list at the App.tsx source (one-line `.filter(d => d.id !== currentId)`) so the NodeInfoTab Autocomplete cannot offer self-reference. Legacy data (a node saved with `link === currentDiagram.id` before this fix) renders gracefully via the existing "Cannot resolve linked diagram with id: <id>" path in `NodePanel.tsx:266-275` (added in `d65f1a9`) — no schema migration needed. Build + 93/93 jest suites / 1009 passes. Closes baseline finding #2. | App.tsx linkedDiagrams effect | manual-test-baseline #2 | low | (this session) |
| B-3 | **[x] `95a0fd5` + reopened-and-restored `4602634` (2026-05-22)** Original fix: removed the custom document-mousedown outside-click listener in [AppToolbar.tsx](../../packages/axoview-app/src/components/AppToolbar.tsx) that closed the share popover whenever the click target wasn't inside the share button's parent. MUI Popover renders into a portal, so clicks on the share URL input + popover body fell outside that container and were treated as outside-clicks. MUI Popover's built-in `onClose` (already wired) handles real outside-click dismissal via its Modal backdrop. **Reopened 2026-05-22:** the B-12 smoke-pass commit (`eb97586`) had deleted the toolbar Share button entirely, retroactively superseding `95a0fd5`. User directed reinstatement of the Share button with a render-disabled + reason-guarded popover pattern in user-facing "server storage" vocabulary — landed as `4602634`. Share button is back; original focus-management bug stays fixed. Closes baseline finding #5. | [AppToolbar.tsx](../../packages/axoview-app/src/components/AppToolbar.tsx) | manual-test-baseline #5 | low-med | (this session) |
| B-4 | **[x] `0c9a1f9`** Resolved earlier (commit `0c9a1f9`, pre-dated this row's filing); verified canonical-complete by exhaustive modifier-key grep 2026-05-22. No Shift+click handler exists; no missing Meta-specific shortcuts. Ctrl/Alt+click drag-pan lives in PanSettings UI per ADR 0008 D5. Added three missing canonical interactions to [HelpDialog.tsx](../../packages/axoview-lib/src/components/HelpDialog/HelpDialog.tsx): **Ctrl+A** (select all visible+unlocked items in active view — useInteractionManager.ts:271), **Alt+click waypoint** (splice from connector — Cursor.ts:174), **Ctrl/Cmd+click** (toggle multi-select on items/connectors — Cursor.ts:418). Hardcoded English strings rather than threading three new keys through the lib's strict-typed `LocaleProps` (would have touched 13 locale files for a discovery dialog). Closes baseline finding #6. | [HelpDialog.tsx](../../packages/axoview-lib/src/components/HelpDialog/HelpDialog.tsx) | manual-test-baseline #6 | low | (this session) |
| B-5 | **[x] `315f395`** Deleted SettingsDialog DiagnosticsTab + the full `onSessionDump` prop chain (App.tsx → Axoview → UiOverlay → SettingsDialog → DiagnosticsTab) + `axoview-app/src/utils/sessionDump.ts`. `enableDebugTools` field stays (still consumed by Renderer + UiOverlay + Axoview as the `__axoview__` exposure gate); `diagnosticsStore` stays (only consumers are the floating DiagnosticsOverlay + DiagnosticsToggleButton, both unaffected). 93/93 jest suites, 1009 passes — identical to pre-edit. | SettingsDialog tabs + onSessionDump chain (diagnosticsStore preserved) | manual-test-baseline #7 | low | (this session) |
| B-6 | **[x] `8ee387a`** Deleted `.github/workflows/docker.yml`; replaced `image: molikas/axoview:latest` + `pull_policy: always` with `build: .` in `compose.yml`; rewrote README "Quick start (Docker)" to the canonical `.env` + `docker compose up --build` flow. `compose.dev.yml` preserved (port mapping + `NODE_ENV=development` still distinguish it). T2 G6 + DOCKERHUB items dropped (see T2 tactical edits in same session). Locks audit Locked Decision #12. | docker.yml + compose.yml + README + T2 tactical | manual-test-baseline #8 | low | (this session) |
| B-7 | **[x] `4bc0005`** Removed nginx HTTP Basic Auth entirely (Locked Decision #13). `nginx.conf`: dropped `auth_basic`, `auth_basic_user_file`, `AUTH_BASIC_SETTING` placeholder, and the nested `location /api/public/` block (the B3 carve-out, now redundant). `docker-entrypoint.sh`: dropped `HTTP_AUTH_USER`/`HTTP_AUTH_PASSWORD` handling, `.htpasswd` write, and the sed-replace. `.env.example`: dropped `HTTP_AUTH_*`. `README.md`: rewrote Quick start without `HTTP_AUTH`; points at `AUTH_MODE` in `docs/deployment.md`. ADR 0009: added Basic Auth removal + B3 rollback note under "Files affected." `docs/deployment.md` needed no changes (already AUTH_MODE-only). Build + 93/93 jest suites / 1009 passes — identical to pre-edit baseline. **BREAKING CHANGE** — deployers using `HTTP_AUTH_*` must switch to `AUTH_MODE=shared-token`. Closes Locked Decision #13, A.6 #D3 final resolution, baseline finding #8 follow-up. | nginx.conf + docker-entrypoint.sh + .env.example + README.md + ADR 0009 | Locked Decision #13 / baseline finding #8 follow-up | med | (this session) |
| B-8 | **[x] Resolved 2026-05-22** — technical fixes landed (`f38d0b4` split each combined `"any maskable"` icon entry in [manifest.json](../../packages/axoview-app/public/manifest.json) into two single-purpose entries so Chrome stops flagging the deprecated combined purpose; both maskable entries reuse the existing 192/512 PNGs — not safe-zone padded but the split removes the manifest-validation warning) + B-12's favicon switch (favicon.svg → favicon-96x96.png, see B-12 row). Three content-creation follow-ups (PWA screenshot at `form_factor: "wide"`, mobile-form-factor screenshot, properly safe-zone-padded maskable PNGs) explicitly deferred to [known_issues.md](../../known_issues.md) per user direction 2026-05-22 — cosmetic, not productization-blocking. | packages/axoview-app/public/manifest.json | B-7 smoke (2026-05-22) | low | (this session) |
| B-9a | **[x] `f843137` (audit) + `1bacd0b` (ADR 0011 Proposed) + `072e924` (S1 application) + this commit (ADR Accepted + bookkeeping)** Error UX contract codified as ADR 0011 (Accepted 2026-05-22). Three deliverables: (1) inventory of 25 error-handling sites (6 already-correct + 20 silent-failure + 5 intentional-silent) — see B-9a investigation findings below; (2) ADR 0011 — every failure-of-intent surfaces an explicit Dialog with locked shape (MUI Dialog + xs/fullWidth + sx styling), per-scenario `<Scenario>ErrorDialog.tsx` naming per ADR 0008 D1, `dialog.<scenario>.*` i18n namespace, dumb-component / smart-parent state ownership; (3) S1 retrofit — public-share load failure now surfaces `PublicShareLoadErrorDialog` instead of a `notificationStore` toast. The two existing dialogs (LocalModeShareErrorDialog, ReadonlyLoadErrorDialog) verified contract-conformant without code edits. Build + 93/93 jest suites / 1009 passes at each code commit. Closes Locked Decision #15. | App.tsx + DiagramLifecycleProvider.tsx + PublicShareLoadErrorDialog (new) + en-US.json + ADR 0011 + this audit doc | B-7 smoke (2026-05-22), session brief, ADR 0011 | med | B-9a |
| B-11 | **[x] `681c7a6`** Preview → Back-to-editing flow surfaced `ReadonlyLoadErrorDialog` on the editor route after the readonly view already rendered correctly. Two defensive changes in [DiagramLifecycleProvider.tsx](../../packages/axoview-app/src/providers/DiagramLifecycleProvider.tsx): cancel-pattern guard around every await checkpoint in the readonly load effect (so late state writes from an in-flight load can't fire after route exit) plus a new effect that clears `readonlyLoadFailed` + `publicShareLoadFailed` whenever `isReadonlyUrl` becomes false. Found during 2026-05-22 smoke pass on the polish trio. | DiagramLifecycleProvider.tsx | 2026-05-22 smoke | low | (this session) |
| B-12 | **[x] `eb97586` (partial — Share-button portion reversed; favicon switch stands)** Toolbar Share button + share popover removed entirely in `eb97586` (intended to canonicalise share via the file-explorer context-menu path) — **reversed 2026-05-22 via B-3 reopening (commit `4602634`)** per user direction; Share button is back with a render-disabled + reason-guarded popover pattern. favicon switch stands as a separate closed finding: 1MB `<svg>` wrapper around a base64 PNG that Chrome's PWA manifest validator flagged as "failed to load" was deleted; index.html, manifest.json, and the AppToolbar brand mark now all point at the 96×96 PNG. `shareUrlFromUuid` + `storage.shareDiagram` retained — still used by DiagramManager + FileExplorer + the reinstated toolbar Share button. Found during 2026-05-22 smoke pass on the polish trio. | AppToolbar.tsx + manifest.json + index.html + favicon.svg (deleted) | 2026-05-22 smoke | low | (this session) |
| B-10 | **Knip soft-fail baseline triage (added 2026-05-22).** T2 G10's first run surfaced: 15 unused files, 3 unused deps, 8 unused devDeps, 2 unlisted deps, 2 unlisted binaries (playwright + wrangler), 1 unresolved import, 23 unused exports, 22 unused exported types, 1 duplicate export. Investigation needed before deletion — some "unused files" may be intentional entry points (i18n bundles, generator scripts). Bundle: investigate-first per A.3 pattern; classify each finding as dead-code (delete) / intentional (suppress via knip config) / orphan-by-cascade (delete with its consumer). Hard-fail promotion gated on a clean run per T2 G10 locked decision. Risk: low. Sessions: 1 (investigation) + 1 (execution). | knip output (all packages) | T2 G10 first soft-fail run (2026-05-22) | low | B-10 |
| B-13 | **[x] `0f08fc9` + `a1f4aa5`** Resolved 2026-05-22 — option (a) executed per user direction; de-DE + id-ID stubs ported to src/i18n; public/i18n/app/ deleted; rsbuild emits canonical bundle (all 13 locales present in build/i18n/app/). **Follow-up `a1f4aa5` (2026-05-22 smoke):** de-DE was registered in `packages/axoview-lib/src/i18n/de-DE.ts` as a valid `LocaleProps` export but never imported into [`packages/axoview-lib/src/i18n/index.ts`](../../packages/axoview-lib/src/i18n/index.ts) — `allLocales['de-DE']` returned undefined, so the lib always received English LocaleProps even when i18next had loaded `de-DE.json` successfully. Wired the import + map entry so the lib now translates German across all surfaces (Settings dialog, sidebar, toolbar). Partial-coverage UX for de-DE + id-ID app-level strings (newer i18next keys post-2026-04) tracked in [known_issues.md](../../known_issues.md) as a separate translation-quality concern. **i18n triple-folder triage (added 2026-05-22, partially resolves A.5.6).** Investigation surfaced 2026-05-22 by the agent during the B-3/Share button work: [packages/axoview-app/public/i18n/app/en-US.json](../../packages/axoview-app/public/i18n/app/en-US.json) (66 lines) is drifted from [packages/axoview-app/src/i18n/en-US.json](../../packages/axoview-app/src/i18n/en-US.json) (129 lines, missing namespaces in the public copy). [rsbuild.config.ts:43-48](../../packages/axoview-app/rsbuild.config.ts#L43-L48) copies `src/i18n/` → `build/i18n/app/`. Investigation findings (build run + file inventory + i18n.ts runtime probe): **Canonical runtime source:** the build output at `build/i18n/app/` is served at runtime — `i18next-http-backend` `loadPath` is `/i18n/{{ns}}/{{lng}}.json` with `ns: ['app']` ([axoview-app/src/i18n.ts:25-28](../../packages/axoview-app/src/i18n.ts#L25-L28)). rsbuild copies `public/` first then applies `output.copy: { from: './src/i18n', to: 'i18n/app' }` second — on collision src/ wins (verified: `build/i18n/app/en-US.json` = 129 lines = src/ version, not 66 lines = public/ version). **public/i18n/app/ status: partially live — NOT dead.** The locale-set diff: `src/i18n/` has 11 locales including pl-PL but missing de-DE + id-ID; `public/i18n/app/` has 12 locales including de-DE + id-ID but missing pl-PL; `build/i18n/app/` is the 13-locale union. The 10 overlap locales are dead-in-public (overwritten by src/); **de-DE.json + id-ID.json are load-bearing in public/** — they're the only source for those two locales, both of which are listed in `supportedLanguages` ([i18n.ts:76-83](../../packages/axoview-app/src/i18n.ts#L76-L83)). Deletion would remove de-DE + id-ID from build → broken locale switching for German + Indonesian users. **Pre-existing partial-locale bug surfaced as side-effect:** since the public/ copies are at the 66-line shape (stale), de-DE + id-ID users already see English fallback for any string post-dating the public/ snapshot — that's a separate quality bug, not a B-13 deliverable. **Recommendation:** reconcile by promoting de-DE + id-ID into `src/i18n/` (treating src/ as canonical), then delete `public/i18n/app/`. Open design question for user: (a) port de-DE + id-ID stale stubs to src/i18n/ + accept the partial-coverage status until translators refresh them, or (b) drop de-DE + id-ID from `supportedLanguages` until proper translations exist (would surface as a UI change — German + Indonesian users currently see them as options). **Self-pause checkpoint fired 2026-05-22** — execution deferred per user decision on (a) vs (b). Risk: low. Session estimate: 1 (execution) once direction chosen; combined with B-10 knip triage if both end up "delete a bunch of files" work. | packages/axoview-app/public/i18n/app/ + packages/axoview-app/src/i18n/ + supportedLanguages | A.5.6 + 2026-05-22 investigation | low | B-13 |
| B-14 | **Node properties panel — redundant "X" close button (added 2026-05-22 from B-13 smoke pass).** The right-side properties panel (Details · Style · Notes tab strip) renders an "X" button that closes the panel. The panel already closes via two other affordances: outside-click on the canvas, and the toolbar's "toggle properties panel" button. Per UX principle: one canonical close gesture per surface; the X is the redundant third. Remove the X from the tab strip and confirm the two remaining affordances cover all cases (including focused-in-panel state where outside-click may be inhibited). Risk: low. Session estimate: 1. | properties-panel header (lib surface) | 2026-05-22 B-13 smoke | low | B-14 |
| B-9b | **Apply ADR 0011 contract to T1-discovered silent-failure surfaces.** Gated on T1 (E2E rewrite) scenario catalogue — the scenarios sharpen the dialog-vs-toast judgement for inline tree operations where a modal Dialog would interrupt the user's recovery context. Working catalogue (20 surfaces from B-9a; each row needs T1-informed disposition between **dialog**, **in-context inline error**, or **stays-as-toast** per ADR 0011 D1 carve-outs): **S2** `DiagramLifecycleProvider.tsx:622` — session-mode `executeSave` storage op failure (toast `alert.saveFailed`). **S3** `DiagramLifecycleProvider.tsx:860` — `handleCreateBlankDiagram` failure (toast 'Failed to create diagram'). **S4** `DiagramLifecycleProvider.tsx:931` — server-mode current-diagram rename failure (toast 'Rename failed' + optimistic-UI rollback). **S5** `DiagramLifecycleProvider.tsx:1002` — toolbar `handleSaveClick` server-mode save failure (toast `alert.saveFailed`). **S6** `DiagramLifecycleProvider.tsx:1061` — `openDiagramById` failure (toast `Failed to open "${name}"`). **S7** `DiagramLifecycleProvider.tsx:1160` — per-id failure inside `saveAllDirty` batch (aggregate toast `${saved}/${failed}`). **S8** `App.tsx:224` — `handleDirectImportFile` empty-tree drag-import failure (toast with `err.message`). **S9** `FileExplorer.tsx:202` — tree-row open failure (toast `Failed to open "${name}"`). **S10** `FileExplorer.tsx:234` — inline create-folder failure (toast 'Failed to create folder'). **S11** `FileExplorer.tsx:248` — inline create-diagram-via-rename failure (toast 'Failed to create diagram'). **S12** `FileExplorer.tsx:268` — tree-row rename failure (toast 'Rename failed' + `tree.refresh()` rollback). **S13** `FileExplorer.tsx:342` — confirm-delete failure (toast 'Delete failed'). **S14** `FileExplorer.tsx:362` — context-menu share-link copy failure (toast with `err.message`). **S15** `FileExplorer.tsx:390` — non-current-row export-image open failure (toast `Failed to open "${name}"`). **S16** `FileExplorer.tsx:409` — direct export-JSON failure (toast `Failed to export "${name}"`). **S17** `FileExplorer.tsx:433` — duplicate-diagram failure (toast 'Duplicate failed'). **S18** `FileExplorer.tsx:480` — drag-drop move failure (toast `Failed to move "${nodeName}"`; drag-context dialog would be jarring — likely stays-as-toast). **S19** `FileExplorer.tsx:493` — post-collision-confirm move failure (toast 'Move failed'). **S20** `DiagramManager.tsx:40,58,83,103` — four legacy-modal catch-arms (toasts; verify whether DiagramManager modal survives D2 cleanup before retrofitting). **Recommendation for T1:** scenarios should cover each row's recovery context — does the user dismiss the failure and stay (in-context inline error), navigate elsewhere (Dialog), or accept the failure and continue (toast)? The dispositions land alongside T1's scenario writing. | App.tsx + DiagramLifecycleProvider.tsx + FileExplorer.tsx + DiagramManager.tsx + per-scenario new dialog files | B-9a inventory (S2–S20) | med | bundle with T1 scenario writing |

#### B-1 investigation findings (2026-05-22)

Code-walk hygiene per design principle 1 — every claim cites `file:line`.

**Surface map:**

- **Routes** ([packages/axoview-app/src/App.tsx:47-51](../../packages/axoview-app/src/App.tsx#L47-L51)) — three routes, all rendering `<EditorPage />`: `/`, `/display/p/:shareUuid` (public share), `/display/:readonlyDiagramId` (owner readonly).
- **Link-picker UI (editable mode)** — [packages/axoview-lib/src/components/ItemControls/NodeControls/NodeInfoTab/NodeInfoTab.tsx:171-199](../../packages/axoview-lib/src/components/ItemControls/NodeControls/NodeInfoTab/NodeInfoTab.tsx#L171-L199). MUI `Autocomplete` over `linkedDiagrams` (full list). Stores `modelItem.link = <diagramId>`. **No preview / open affordance** — picker is set-only.
- **Linked-diagram preview link (readonly mode only)** — [packages/axoview-lib/src/components/ItemControls/NodeControls/NodePanel/NodePanel.tsx:241-277](../../packages/axoview-lib/src/components/ItemControls/NodeControls/NodePanel/NodePanel.tsx#L241-L277). Constructs `href={\`/display/${linkedDiagramId}\`}`. Only rendered when the panel is in `readOnly` mode (i.e., when `editorMode === 'EXPLORABLE_READONLY'`, set by App.tsx:222 when `isReadonlyUrl`). Per d65f1a9 (MQA #22/#25), this section was deliberately scoped to the read-only panel.
- **Toolbar "Preview" button** — [packages/axoview-app/src/components/AppToolbar.tsx:222-240](../../packages/axoview-app/src/components/AppToolbar.tsx#L222-L240). Calls `handlePreviewClick` which opens `/display/${currentDiagram.id}` in a new tab ([DiagramLifecycleProvider.tsx:1059-1063](../../packages/axoview-app/src/providers/DiagramLifecycleProvider.tsx#L1059-L1063)). Note: this is current-diagram preview, NOT linked-diagram. Disabled unless `serverStorageAvailable && currentDiagramId`.
- **`isReadonlyUrl` computation** — [DiagramLifecycleProvider.tsx:155-158](../../packages/axoview-app/src/providers/DiagramLifecycleProvider.tsx#L155-L158): `isPublicShareUrl || (window.location.pathname.startsWith('/display/') && !!readonlyDiagramId)`.
- **Public-share load path** — [DiagramLifecycleProvider.tsx:370-419](../../packages/axoview-app/src/providers/DiagramLifecycleProvider.tsx#L370-L419). Loads icon packs via `iconPackManager.loadPacksForDiagram(data)`, merges colors with `defaultColors`, sets `frozenInitialDataRef.current` when axoview not yet mounted, calls `axoviewRef.current?.load(...)`. On failure: pushes a notification (no redirect).
- **Owner-readonly load path** — [DiagramLifecycleProvider.tsx:424-461](../../packages/axoview-app/src/providers/DiagramLifecycleProvider.tsx#L424-L461). Calls `storage.listDiagrams()` + `storage.loadDiagram(readonlyDiagramId)`. **Missing parity** with the public-share path: no icon-pack load, no defaultColors merge, no `frozenInitialDataRef.current` fallback. **On failure: `window.location.href = '/'`** (line 457) — silently redirects to root and resumes whatever diagram was last open.
- **Local-mode share-error dialog** — [App.tsx:208](../../packages/axoview-app/src/App.tsx#L208) + [LocalModeShareErrorDialog.tsx](../../packages/axoview-app/src/components/LocalModeShareErrorDialog.tsx). Triggered by `isReadonlyUrl && !serverStorageAvailable`. **Blanket** — fires for BOTH `/display/p/<uuid>` AND `/display/<diagramId>`, even though only the share form is genuinely session-only (Local mode could in principle serve owner-readonly for a diagram that exists in `localStorage`).

**Hypotheses:**

- **Why Bug A (no affordance in Local):** The preview link exists only in the read-only `NodePanel` branch (NodePanel.tsx:121-294). In editable mode the picker (NodeInfoTab.tsx:170-199) just sets `modelItem.link` — there is no "open this link" button anywhere. To navigate to the linked diagram a user would need to manually visit `/display/<id>`, which in Local mode is blocked by `LocalModeShareErrorDialog`. **Net effect: the feature is unreachable in Local mode.** This is a missing-UI bug, not a routing bug.
- **Why Bug B (preview opens edit mode in Session):** The toolbar "Preview" button (AppToolbar.tsx:233) opens `/display/${currentDiagram.id}` in a new tab. In the new tab, the owner-readonly effect (DiagramLifecycleProvider.tsx:424) fires when storage initializes. If `storage.loadDiagram(id)` throws — 404, transient network failure, etc. — the catch at line 452 runs `window.location.href = '/'`, which reloads the SPA at root and resumes the last-opened diagram **in edit mode**. The user perceives this as "preview opened the same diagram in edit mode." The notification toast is the only signal the load failed, and it's easy to miss on a tab reload. The most likely failure causes are auth/cookie scope on the new tab or an ID-format mismatch (see Bug C).
- **Why Bug C (`/display/<diagramId>` form broken):** Two contributing factors. **(i) Same `window.location.href = '/'` cascade as Bug B** — any load failure silently redirects to root. **(ii) ID-format mismatch:** the user's broken URL is `/display/diagram_1775916857851` (decimal `Date.now()`), but the current backend generates IDs as `diagram_<base36>_<rand>` ([routes.js:105](../../packages/axoview-backend/src/routes.js#L105)) and the current `LocalStorageProvider` uses `diagram_<base36>_<rand>` ([LocalStorageProvider.ts:85-90](../../packages/axoview-app/src/services/storage/providers/LocalStorageProvider.ts#L85-L90)). The decimal-suffix form is a **legacy ID** — either persisted from an older app version, or from `Date.now()` directly before the `toString(36)` change. If `modelItem.link` was set in that older era and the linked diagram has since been recreated with a new-format ID, the `/api/diagrams/<legacy_id>` lookup 404s, the catch fires, and the redirect cascade hits Bug B. **Equally plausible:** the linked diagram was deleted but the link reference wasn't cleaned up — same 404 → same redirect.

**The unifying root cause:** the owner-readonly load path's catch handler swallows the error and redirects to `/`. Every observable symptom in Bugs B + C comes from that single line. Bug A is a separate missing-UI issue (no preview affordance in edit mode).

**Proposed fix:**

1. **NodeInfoTab.tsx** — add a small "open linked diagram" `IconButton` next to the `Autocomplete` (visible when `modelItem.link` is set). Behaviour TBD per design fork below.
2. **NodePanel.tsx** readonly branch — leave the `href={\`/display/${linkedDiagramId}\`}` link as-is; it's the correct URL form for the owner-readonly route. Routing works (Bug C's root cause is in the load path, not the route definition).
3. **DiagramLifecycleProvider.tsx owner-readonly path** — two changes:
   - **Add parity with the public-share path:** call `iconPackManager.loadPacksForDiagram(data)`, merge `defaultColors`, and set `frozenInitialDataRef.current` when `axoviewRef.current` is null (so the first render of a freshly-opened tab gets the readonly data even before `axoviewRef.current?.load()` is callable).
   - **Replace the `window.location.href = '/'` catch handler** with an explicit error UI. Mirror the `LocalModeShareErrorDialog` pattern (B2 / commit cff3942): a `ReadonlyLoadErrorDialog` that says "Could not load this diagram (it may have been deleted, or you don't have access)." with a "Back to editor" button that navigates to `/` via React Router (not `window.location`). This addresses B-9(a) at the same time.
4. **App.tsx `showLocalModeShareError` logic** — narrow from "any `/display/*` URL in Local mode" to "share-UUID URLs in Local mode only" (i.e., trigger on `isPublicShareUrl && !serverStorageAvailable`, not on the broader `isReadonlyUrl && !serverStorageAvailable`). This unblocks Local mode from rendering `/display/<diagramId>` for diagrams that exist in `localStorage`. **This is the contentious step** — it changes ADR 0009 D3's interpretation (see design fork below).
5. **NodeInfoTab.tsx** B-2: filter the current diagram out of the Autocomplete `options` (need `currentDiagramId` injected via props or a new `useUiStateStore` field).

**Scope estimate:** 4-5 files, ~120-180 LOC including a new `ReadonlyLoadErrorDialog` component + a new i18n string set. **Borderline single-PR scope; close to the self-pause threshold.**

**Design fork — requires user decision before commit 2:**

The Local-mode preview affordance has two materially-different shapes. The user has not blessed either, and the example in the session brief literally names this question.

- **Option A — "Open inline" (switch to linked diagram in the same tab).** The button in NodeInfoTab calls `openDiagramById(linkedId, linkedName)` — the same flow the file explorer uses. No new tab, no `/display/*` URL involved. Works identically in Local + Session. Side-effects: current diagram's unsaved changes need to be handled (in Local mode this becomes a "switch buffer" via the existing `scratchBufferRef` machinery). **Pro:** no routing changes, no ADR 0009 D3 reinterpretation, smallest scope. **Con:** "preview" semantically implies a separate view (you don't lose your place), and inline switch loses the current diagram from view — the user explicitly said "click link → opens B" in J5, which reads as "opens B somewhere I can see it without losing A."
- **Option B — "Open in new tab" via `/display/<id>`.** Matches Bug B's intent + the existing readonly `NodePanel` link. Requires step 4 above (narrowing `showLocalModeShareError` to share-UUIDs only) so Local mode tabs can render `/display/<id>` against `localStorage`. ADR 0009 D3 says "share routes are session-mode only" — the question is whether `/display/<diagramId>` (owner readonly) is a "share route" or just an "owner readonly route" that happens to share the URL prefix. The latter reading is consistent with the route name (`readonlyDiagramId`) and with the data path (`storage.loadDiagram`, which exists in Local mode too). **Pro:** matches user mental model ("preview = new tab, separate view"); fixes Bug C in Local mode in passing; aligns NodeInfoTab affordance with NodePanel readonly affordance. **Con:** broader scope; touches App.tsx + an ADR interpretation note; requires verifying that `localStorage.loadDiagram(id)` works correctly on a fresh tab (i.e., that the SPA initializes Local mode + reads the same localStorage in the new tab — it does, since localStorage is origin-scoped, but worth confirming with the manual smoke).

**Recommendation:** Option B. Reasoning: the user's verbatim J5 spec is "click link → opens B" — they describe a navigation, not an inline switch. Option B is also the only one that fixes Bug C as a side-effect. Cost: a one-paragraph addendum to ADR 0009 clarifying the share vs. owner-readonly distinction within the `/display/*` namespace.

**Scope:** **single-PR — pending design decision.** If user blesses Option B, commits 2 + 3 land per the session plan. If user prefers Option A, commit 2 narrows substantially (no App.tsx change, no ADR addendum, no `ReadonlyLoadErrorDialog`); Bug C resolution then folds back into B-9 (SPA error UX polish) since the catch-handler fix is the same machinery.
...
**Actual outcome (2026-05-23):** M10 met (v1.0.0 shipped); Findings register fully mined; spawned tacticals all wrapped (T1 E2E complete Session 8; T2 git-automation complete 2026-05-22; T3 lib deletion absorbed into the cleanup waves; T4 remains as user-external dashboard actions). `flare_plan.md` was deleted **pre-audit** in commit `926e66f` (confirmed by A.6.1); its durable decisions live in ADRs 0009 + 0010, both Accepted 2026-05-20. The `project_flare_plan.md` memory entry was retired during C.6 (2026-05-20). The original wrap-up plan also presumed an ADR 0007 (trace harness) artifact — Phase B's `?trace=1` work shipped operationally but the ADR was never authored; the harness lives via its commits, not a formal record.

Per Locked Decision #17, closeout sequencing for the steps remaining after M10:

1. **(a) Cleanup session (this commit set, 2026-05-23):** README FossFLOW sweep + post-audit drift fixes; semantic-release `successComment` disable; this Wrap-up touch-up.
2. **(b) Technical review artifact** at `docs/technical-review-2026-05.md` — 3-session snapshot for an independent reviewer: comprehensive every-git-tracked-file inventory + Mermaid sequence diagrams + dual-lens reviewer prompts. This artifact was not in the original plan; it captures shipped reality for someone joining cold.
3. **(c) T4 user external actions (out-of-repo):** GitHub Settings → Branches → branch protection on `master` (require PR + status checks: Run Tests / E2E Tests (Playwright) / CodeQL); Settings → General → set Description, Topics (`isometric`, `react`, `diagram-editor`, `cloudflare-pages`, `docker`), Homepage URL (`https://axoview.pages.dev/`). CodeQL repo-toggle already done.
4. **(d) `/feature` wrap productization-audit ceremony:**
   - Add a single line under `PLAN.md` Phase 2D section (or a new "Post-2D productization" sub-line if needed):
     ```
     - Productization audit complete (v1.0.0 shipped 2026-05-23) — see docs/adr/0008 (naming), 0009 (deployment topology), 0010 (session backend), 0011 (error UX), and spawned tactical histories (cleanup, E2E rewrite, git-automation hardening). flare_plan.md retired pre-audit; trace harness shipped without a formal ADR.
     ```
   - Confirm all spawned tactical docs are wrapped.
   - Delete this audit doc. ADRs 0008–0011 (+ further) + the technical review artifact + the wrapped tactical histories are the durable record.

The audit doc stays present until step (d) — useful as a sanity-check artifact during the technical review window.
