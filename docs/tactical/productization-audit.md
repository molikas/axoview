...
| C.8 — Git-automation tactical | **in progress 2026-05-21+** — G9 (`8ee387a` 2026-05-21) + G2 (`967b2c7` 2026-05-22) done; G5 dropped 2026-05-22 per Locked Decision #14; G1, G3, G4, G7, G8, G10 remain (six rows). | [git-automation-hardening.md](git-automation-hardening.md) — see T2 row in C.2 Section 4 for the per-row status |
...
| T2 | **C.8 — Git automation hardening** | Add ESLint to CI (closes A.8 #A1); remove `\|\| npm test` coverage fallback (closes A.8 #A2); add `commitlint` + simple-git-hooks (closes A.8 #A7); add `codeql.yml` (closes A.8 #A8 + #A4); ~~add Cloudflare Pages deploy automation~~ (G5 dropped 2026-05-22 per Locked Decision #14); add container scanning (G6 dropped 2026-05-21 per Locked Decision #12); add CI bundle-size check for Worker per ADR 0009 Decision 8; add CI verification that `_routes.json` + `_headers` are emitted by build per ADR 0009 Decision 5. **Gated on Phase C ADRs accepted (now satisfied).** | A.8 #A1, #A2, #A4, #A5, #A7, #A8, ADR 0009 | **in progress — G9 `8ee387a` (2026-05-21, compose-build fallback) + G2 `967b2c7` (2026-05-22, coverage-gate fallback removed) done; G5 dropped 2026-05-22 per Locked Decision #14. Remaining: G1 (ESLint in CI), G3 (commitlint + simple-git-hooks), G4 (CodeQL), G7 (Worker bundle-size), G8 (build-output verification), G10 (continuous knip) — six rows, shippable in 1–2 sessions. See [git-automation-hardening.md](git-automation-hardening.md).** |
