...
|---|---|---|---|---|
| T1 | **C.5 — E2E rewrite (Playwright)** | New `packages/axoview-e2e/` (existing one slated for deletion per audit-locked-decision #4); new `.github/workflows/e2e-playwright.yml` to replace the Python suite; surface-anchor sweep using `data-axoview-id` per ADR 0008 Decision 5. **Gated on M9.** | locked-decision #4, ADR 0008 Decision 5 | **[~] smoke set complete 2026-05-22** — Sessions 2 + 3 shipped. **Session 2 (foundation, 5 commits):** `3ff4110` delete legacy suite, `3f087c8` skeleton + config, `62d9705` fixtures/helpers, `cce1dda` AppToolbarPOM + J1 smoke green, `516ba22` doc-sync. **Session 3 (smoke completion, 4 commits):** `ddb14d7` J20 EmptyState extension (+ EmptyStateScreenPOM + `screen-empty-import` retrofit + workers=1 pin), `f611aa2` J2 connector spec (+ `canvas-interactions` lib retrofit + `getModelConnectorCount` helper), `67571c4` J15 hotkeys spec (+ `getViewItemCount` helper), this commit (bookkeeping). Suite now at 11 tests / 4 spec files, all green locally in ~2.6min. Six lazy `data-axoview-id` retrofits running total (toolbar-save, screen-empty-create, screen-empty-import, dock-elements-toggle, dock-layers-toggle, canvas-icon-grid-item, canvas-interactions). Sessions 4–6 (file ops, editor surfaces, diagram-link + dialogs) remain. |
