...
|---|---|---|---|
| `entry`/`exit` | cursor `crosshair` / `default` | same | [Connector.ts:150-155](../../packages/axoview-lib/src/interaction/modes/Connector.ts#L150-L155) |
| `mousedown` | 1st press → `handleClickFirst`: `beginDragTransaction` + `createConnector`, arm `isConnecting` with `startAnchor` = the node under the cursor **or the raw tile** (free-floating start is allowed — ADR 0022 addendum; a stray empty click is reverted on `mouseup`, not blocked here). 2nd press → `handleClickSecond`: set anchor[1] to target, `commitDragTransaction`, return to CURSOR or reset. | `handleDragStart`: `beginDragTransaction` + `createConnector`, hold `id`. | [L205-226](../../packages/axoview-lib/src/interaction/modes/Connector.ts#L205-L226) |
