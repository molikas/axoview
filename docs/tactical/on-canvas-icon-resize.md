...
- **"Trace each shape" outline rule (2026-07-19, owner-chosen).** The rim traces the element's real shape: flat elements (rectangles, text boxes, **flat/Material icons**) → iso diamond ([`TransformControls`](../../packages/axoview-lib/src/components/TransformControlsManager/TransformControls.tsx)); standing isometric icons → screen box ([`ScreenBoxTransformControls`](../../packages/axoview-lib/src/components/TransformControlsManager/ScreenBoxTransformControls.tsx)). [`NodeTransformControls`](../../packages/axoview-lib/src/components/TransformControlsManager/NodeTransformControls.tsx) + `HoverNode` branch on `icon.isIsometric`. Hover extended to text boxes ([`HoverOutline`](../../packages/axoview-lib/src/components/TransformControlsManager/HoverOutline.tsx) `HoverTextBox`); labels/connectors stay excluded. Readout made screen-stable on both node shapes. See the ADR 0044 third addendum. **Should fold into [ux-principles §4.2](../guidelines/ux-principles.md) / [ADR 0006 §9](../adr/0006-canvas-selection-contract.md) at wrap.**
- **Material-icon (non-iso) fixes (2026-07-19).** (1) The outline branch now matches the renderer's `!icon.isIsometric` — `isIsometric` is optional, and an undefined-flag icon renders flat, so it must get the iso diamond, not the screen box (which can't follow a sheared flat glyph in iso). (2) The flat icon now grows about its **scale-1 centre** in BOTH the DOM ([`NonIsometricIcon`](../../packages/axoview-lib/src/components/SceneLayers/Nodes/Node/IconTypes/NonIsometricIcon.tsx): `transform: scale()` about centre) and the WebGL bulk ([`NodesCanvas`](../../packages/axoview-lib/src/components/SceneLayers/Nodes/NodesCanvas.tsx) non-iso-iso branch: origin shifted back by half the extra sheared extent), so a resize expands symmetrically instead of only down-and-right from the top-left corner (matching the isometric branch). scale-1 render is byte-for-byte unchanged, so the two paths stay in lockstep (no jump on select/deselect).
