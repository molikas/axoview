...
|---|---|---|---|---|---|
| G1 | **Add an ESLint step to [.github/workflows/test.yml](../../.github/workflows/test.yml).** Extend the existing matrix job: insert `npx eslint .` (or `npm run lint` if the root script is rewired to invoke ESLint) between `npm ci` and `npm test`. Verify by deliberately pushing a lint-error commit on a throwaway branch and confirming the workflow fails red. **G1 lint baseline (2026-05-22):** Errors: **0**. Warnings: **196**. Top rules: `@typescript-eslint/no-explicit-any`: 133, `@typescript-eslint/no-unused-vars`: 30, `react-hooks/exhaustive-deps`: 26, `no-console`: 4. Distribution: lib=92, app=104, worker=0, backend=0, e2e=0. **Recommended approach:** add `npx eslint .` as a hard-fail step (default ESLint behaviour: any error → non-zero exit). With zero errors in the tree, the step is green on first run; the 196 warnings stay informational and can be ratcheted down in a future cleanup wave without blocking M10. No `--max-warnings` flag added (would flip warnings to errors and break CI immediately). | `test.yml` | A.8 #A1 | medium | [ ] |
