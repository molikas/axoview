...
> **Status:** Frozen-on-publish 2026-06-10 (post-v1.1 wave, v2.0.1). Reviewer-ready. This document is a snapshot of the post-v1.1 state — it does not track ongoing changes; the [ADRs](adr/), [PLAN.md](../PLAN.md), and [docs/architecture.md](architecture.md) are the living artifacts. The prior [2026-05 review](technical-review-2026-05.md) is preserved unedited as the v1.0.0 snapshot; this artifact tracks the delta since.
...
**Snapshot date.** This artifact is dated **2026-06-10** and is **not a living doc**. It captures the state at the close of the **v1.1 quality wave** (PRs #3–#23 against the v1.0.0 baseline; release `v2.0.1`). The ADRs, [PLAN.md](../PLAN.md), and [docs/architecture.md](architecture.md) are the living artifacts; this one is a frozen review surface, published once and not edited in place (errors found later go in a future `## 12. Corrections` appendix, the convention the [2026-05 artifact](technical-review-2026-05.md#12-corrections-added-2026-05-25) used).
...
| **Version** | `v1.0.0` (semantic-release) | `v2.0.1` (root/lib/app/backend); worker still `1.0.0` | git tags, package.json files |
...
Unchanged by the v1.1 wave. The lib carries four Zustand stores; the app carries one more (notifications). All five use the React-context-wrapped Zustand pattern (per-mount-instance isolation) — see [architecture.md §2a](architecture.md).
...
**11 tracked files** (was 5). [`app.ts`](../packages/axoview-worker/src/app.ts) is 54 LOC (the `onError` handler added 10), [`auth.ts`](../packages/axoview-worker/src/auth.ts) 220. **4 suites / 102 tests.** The runtime decoupling the 2026-05 §12 B2 correction named still holds: the Worker does *not* import `routes.js` at runtime; the `../axoview-backend/src/**/*` coupling in `tsconfig.json` is typecheck-only. Still versioned `1.0.0` while the monorepo is `2.0.1` ([§11](#11-open-known-issues)).
...
9. **Version coherence.** [§11](#11-open-known-issues) flags `axoview-worker` at `1.0.0` while the monorepo is `2.0.1`. Trace why semantic-release didn't bump it. Is this a packaging bug (worker excluded from the release config) or intentional (worker not independently versioned)? What breaks, if anything?
...
4. **Worker version drift as a release-hygiene smell.** `axoview-worker@1.0.0` vs `2.0.1` everywhere else. If the deployed CF Worker reports its own version anywhere, this is operator-visible confusion. Recommend the fix (add to release config, or document the worker as intentionally unversioned) and whether it should gate the next release.
...
**Worker version drift — `axoview-worker@1.0.0` vs `2.0.1` everywhere else.** *Severity: med.* The root + lib + app + backend are all `2.0.1`; the Worker package is still `1.0.0`. semantic-release bumped the others (the `v2.0.0` major came from the lib's `mainMenuOptions` `BREAKING CHANGE` footer) but not the worker. Either the worker is excluded from the release config or it is intentionally unversioned — undecided. Operator-visible if the deployed Worker ever reports its own version. *Next-action: user-call — fix the release config or document the worker as unversioned.*
