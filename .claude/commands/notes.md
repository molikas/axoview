...
>
> **Linking ADRs:** when summarising the session's durable output (architecture.md cross-refs, README updates), link the relevant ADRs. **Read the `**Status:**` header of the ADRs you cite** — [docs/adr/](../../docs/adr/) is the source of truth and the set grows every wave (41 as of 2026-07-15, numbered to 0043; 0016/0017 unused). Never cite an ADR as shipped without checking its Status — **grep the headers, don't trust any list** (including this one). *(This note has twice gone stale by hardcoding an enumeration: first a 10-ADR list that called ADR 0007 an un-shipped placeholder, then a "still `Proposed` (0022, 0023, 0025, 0028)" list that was falsified by the same 2026-07-15 sweep that flipped all four to `Accepted`. There are currently **no** `Proposed` ADRs and one superseded in part — but verify that with `grep -rh '^\*\*Status:\*\*' docs/adr/ | sort | uniq -c` rather than believing this sentence.)*
...
3. `docs/guidelines/testing.md` — header line (total tests + suites).
...
| `docs/guidelines/architecture.md` | New subsystem, new pattern, new known issue, or "gotcha" in commit bodies |
...
Report the scope to the user in one line before Phase 1: *"I'll update: CHANGELOG, docs/guidelines/architecture.md, docs/guidelines/testing.md. Skipping: README (no new user-facing features), known_issues.md (nothing new), PLAN.md (no phase completed)."*
...
Anything non-obvious that `docs/guidelines/architecture.md` needs to capture — a hidden constraint, a new invariant, a gotcha? Code explains *what*; this captures *why*.
