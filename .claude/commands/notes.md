...
- `update-version` bumps all 5 `package.json` files and `@semantic-release/changelog` regenerates `CHANGELOG.md` **inside the CI build (not committed back — branch protection, see commit #77)**; a `vX.Y.Z` tag + GitHub Release land automatically. The in-app version is injected from the tag at build time (ADR 0045).
