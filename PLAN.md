...
**v1.1 Track 7 — new-audit-workstreams backlog (catalogued, *not yet authorized*; each row spawns its own tactical when prioritized):**

These 20 rows are the "new audit workstreams" referenced in the v1.1 tech-debt cleanup entry (Completed-alongside-2D above). They lived only in `docs/tactical/v1.1-tech-debt.md` (Track 7 table), retired in commit `1cb4314` — enumerated here so they survive the tactical's deletion. Disposition column: *Open* = no work started; *Defer* = parked pending a trigger; *§N* = also tracked in [`docs/technical-review-2026-06.md`](docs/technical-review-2026-06.md). Authorization gate: each row spawns a separate tactical via `/feature start` when the user prioritizes it.

| # | Workstream | Suggested vehicle | Disposition |
|---|---|---|---|
| 1 | **M5 XSS surface** — `dangerouslySetInnerHTML` in `TextBox.tsx` / `useTextBoxProps.ts` / `config.ts` + CSP `'unsafe-inline'` style-src | Polish ADR + tactical (Quill review × CSP tightening) | Open · §6d/§11 |
| 2 | **M8a accessibility baseline** — zero ARIA / WCAG / keyboard-trap audit | Standalone tactical | Open · feeds Sonar S7924 + `FileExplorer.tsx:63` keyboard listener |
| 3 | **M8b telemetry / privacy disclosure** — `connect-src googleapis.com` pre-Drive; no privacy policy; PWA manifest has no data-handling description | Privacy ADR | Open |
| 4 | **M8c v1.0.0 → v1.x migration policy** — semantic-release shipping minors against undocumented data-shape | Versioning ADR | Open |
| 5 | **M8d SBOM + SRI** — OAuth scripts whitelisted in CSP have no integrity hashes | Supply-chain ADR | Open |
| 6 | **B-9b silent-failure retrofit (S2–S20)** | Opportunistic per-PR, **not a sweep** (LD #5) | Partial — ADR 0011 dialogs landed (`PublicShareLoadErrorDialog` / `ReadonlyLoadErrorDialog`); remainder folds into deferred-features row 1 |
| 7 | **Express morgan / structured logging** | Observability ADR | Open · §11 "awaits real user complaint" |
| 8 | **`*.pages.dev` preview-deploy warning when `AUTH_MODE=none`** | Deploy-time warning or preview block | Open · §11 |
| 9 | **Worker `wrangler.toml` consolidation** | Symlink or generator script | Open · §11/§5f |
| 10 | **CSP `'unsafe-inline'` style-src tightening** | MUI emotion nonce/hash work | Open · §11/§6d |
| 11 | **ADR 0007 (trace harness) backfill** | Authorial backfill from commits | Open · §9.1 |
| 12 | **i18n triplication coverage-matrix CI linter** | Linter for lib × app coverage matrix | Open · §7.8 |
| 13 | **`axoview-lib` public-API surface narrowing** | Public-API audit if npm-publish need surfaces | Open · §7.1 |
| 14 | **MQA #21 folders-collision repro** | Defer | Open · `known_issues.md` |
| 15 | **Connector drag sustained-GC cliff (~50s marathon drags)** | Defer | Open · §11 |
| 16 | **MQA diag exporter element counts read 0** | Defer | Open · §11 / `DiagnosticsOverlay.tsx` |
| 17 | **Context-menu surface decision** — **narrowed 2026-06-10**: the `ContextMenu.tsx` renderer was deleted in PR #24's Knip pass; only the `ContextMenuManager` no-op stub remains, still rendered at `UiOverlay.tsx:298` with `setContextMenu()` plumbing in `useInteractionManager`. Decide: wire a renderer OR remove the stub + state + setter entirely. | ADR or feature tactical | Open · = PR #24 finding #2 |
| 18 | **CodeQL config drift** — verify `.github/workflows/codeql.yml` languages list / query suites / schedule match what the repo exercises (the repo *toggle* is closed per §11; the config *audit* is not) | Light audit / spawned tactical | Open |
| 19 | **`uiOverlay.editorModes.test.ts` ZOOM_CONTROLS mirror drift** — test-mirror's local `EDITOR_MODE_MAPPING` diverges from prod for ZOOM_CONTROLS visibility | Single-commit fix or fold into next test-cleanup wave | Open · same `__perf_refactor_regression__` folder as the §11 `tsc` drift |
| 20 | **`no-explicit-any` baseline elimination** | — | ✅ **SHIPPED** — PRs #13–#15 (see catalogued entry above) |

**Authorization gate:** each row spawns a separate tactical via orchestrator / `/feature start` when the user prioritizes it. No row is authorized by being listed here.

**v1.1 PR-session findings (smaller catalogued items surfaced during the v1.1 wave PRs):**

- **PaperProps → slotProps migration** — 11 MUI dialogs/popovers still pass the deprecated `PaperProps` prop (`ConfirmDialog`, `ExportPopover`, `ExportSingleDiagramDialog`, `FileExplorer` ×2, `ImportDialog`, `LoadDialog`, `PublicShareLoadErrorDialog`, `ReadonlyLoadErrorDialog`, `SaveDialog`, lib `DeleteIconConfirmDialog`). PR #14 migrated only the 2 sites that also carried `as any`; the rest remain. **This is a concrete instance of Sonar S1874** (deprecated APIs, below) — fold into that sweep. Source: PR #14 (2026-06).
- **Duplicate `SavedDiagram` interface** — declared twice: local `interface SavedDiagram` at `LoadDialog.tsx:22` and the exported `interface SavedDiagram` at `DiagramLifecycleProvider.tsx:66`. Consolidate (`LoadDialog` imports the exported one). Minor improvement; single-commit. Source: PR #14 (2026-06).
- **Perf-regression `tsc --noEmit` type drift** — the ~17 strict-type errors in `axoview-lib/src/__perf_refactor_regression__/*.test.ts(x)` are **already tracked** in [`docs/technical-review-2026-06.md`](docs/technical-review-2026-06.md) §11 ("New in this review"); cross-referenced here, not re-catalogued. Overlaps Track 7 #19. Source: PR #16 (2026-06).

**v1.1 Sonar Tier 2 remaining backlog (catalogued; M1 shipped the bug-class sites in PR #16):**

The Sonar Tier 1 refactor (S3776/S2004) and the M1 Tier 2 bug sweep (S2486 empty-catch / S1854 dead-store / S6479 array-index-key, PR #16) are done. The remaining Tier 2 rule-classes are quality / modernization, not bugs — each is a separate sweep when prioritized:

| Rule | Class | Sites | Note |
|---|---|---|---|
| S1874 | Deprecated APIs | ~36 | Includes the 11 `PaperProps` sites above |
| S3358 | Nested ternaries | 36 | Readability; per-site judgment |
| S6759 | React props should be read-only | 19 | Mechanical; one PR |
| S7924 | CSS contrast (a11y) | 9 | **Flag for design input** — feeds Track 7 #2 (a11y baseline) |

Source: local Sonar scan (Tier 2 residue after the 2026-06-09 → 10 refactor wave).
