...
> Last updated: 2026-07-15 (docs housekeeping. **Perf strategy reversed since the last stamp:** the 2026-06-25 "continue evolving Canvas2D, no engine rewrite" decision was overturned — **ENG-T4 (WebGL2) shipped 2026-07-08** as the sole bulk substrate ([ADR 0038](docs/adr/0038-webgl-instanced-render-substrate.md), v3.5.0 / PR #63), which also **closed ENG-PAN/R1** outright. ENG-T3 (sim/LEB60) stays deferred, now gated on a *WebGL2* ceiling rather than a Canvas2D one — and the perf charter that carried its mandate was **wrapped**, so the ENG-T3 row below is now that mandate's only home. Released line: **v3.7.0** (2026-07-14) — Drive-native sharing ([ADR 0042](docs/adr/0042-drive-native-sharing-and-readonly-preview.md) / [0043](docs/adr/0043-deferred-backend-for-google-api-hardening.md)) shipped in it via PR #69. Durable perf protocol: [ADR 0020](docs/adr/0020-engine-perf-harness-and-measurement-protocol.md) + `perf-results/decision-log.md`.)
...
| **3B** | Google Drive Provider (E4) | `[x]` | High | Shipped 2026-07-06 — [ADR 0036](docs/adr/0036-google-drive-storage-provider.md) + [ADR 0037](docs/adr/0037-storage-places-model.md) places model (one tree / two places, move + bulk migration). Deferred: offline queue, reverse move (known_issues). **Drive-native sharing + read-only preview shipped 2026-07-14** — share affordance in Drive mode + custom Drive-REST permissions dialog + `/display/drive/:id` **server read-proxy** (anonymous "anyone with the link" preview, no login) + Drive-trash-honoring; see [ADR 0042](docs/adr/0042-drive-native-sharing-and-readonly-preview.md) (Accepted, supersedes 0036 §4) + [ADR 0043](docs/adr/0043-deferred-backend-for-google-api-hardening.md) (deferred backend) + git history. Picker per-file grant (Option B) deferred to v1.1. |
...
| **5***  | Cloudflare + Docker dual-target deploy | `[x]` | High | See [docs/deployment.md](docs/deployment.md). **Marketing landing + SEO/social metadata shipped 2026-07-13** — indexed root `/`, editor moved to `/app` (SPA→`app.html`, router basename `/app`), OG/Twitter/JSON-LD/sitemap/graceful-404; see [ADRs 0040–0041](docs/adr/) (both Accepted) + retired `docs/tactical/seo-and-marketing-landing.md` git history. **Docker regression gate shipped 2026-09-24** — required `Docker Gate` smoke + `Docker Regression Gate` full E2E against the built image; see [ADR 0048](docs/adr/0048-docker-image-regression-gate.md) + retired `docs/tactical/docker-regression-gate.md` git history. |
...
| **ENG-T2** | Engine perf — Canvas2D node render | `[x]` ⤴ superseded | ⚠️ Very High | Node layer → Canvas2D (default, flag removed): spawn −41% @1000, scales to ~2,000 (SSB). See [ADR 0019](docs/adr/0019-canvas2d-node-render-layer.md) + harness/protocol [ADR 0020](docs/adr/0020-engine-perf-harness-and-measurement-protocol.md). **Superseded 2026-07-08 by ENG-T4 (WebGL2):** the Canvas2D bulk layer and its fallback were removed, and ADR 0019 is *superseded in part* by [ADR 0038](docs/adr/0038-webgl-instanced-render-substrate.md). Kept `[x]` as the historical step that got us to T4. |
| **ENG-T2.5** | Pre-T3 hardening — paste O(N) + render/drag parity | `[x]` | ⚠️ Very High | Paste freeze O(N³)→O(N+C) (validation hoisted out of the per-node loop); derived spatial `TileIndex`; rigid-stamp collision; rectangle/textbox compositor drag (CSS var, single commit on drop); canvas label stalk + LOD + text-layout cache; O(N²)→O(N) canvas item lookup; honest connector-paint harness anti-cheat + `perf-smoke` CI gate; scoped post-undo/redo connector re-sync (D-8). See [ADR 0021](docs/adr/0021-paste-algorithmic-perf-and-spatial-index.md). PR #48 (paste) + #49 (pre-T3). |
| **UX-CANVAS** | Canvas UX overhaul (interaction · selection · off-grid · labels · export) | `[x]` | ⚠️ Very High | 20 issues / 9 tracks + perf gate. Opinionated interaction model (single-click select, double-click open, right-drag pan, context menu), lasso intersection + endpoint capture, off-grid placement + per-item collision, label direct-drag, rectangle edge handles, robust export. Held the engine perf budget (Track P / [ADR 0020](docs/adr/0020-engine-perf-harness-and-measurement-protocol.md): spawn neutral tip-vs-pre, KR1 7.8%, T6 label-drag fixed 103→17.5 ms). See [ADRs 0022–0027](docs/adr/) + ADR 0006/0013 addenda + this file's git history. UX re-test fixes shipped (2026-06-21 journey-tests, [ADR 0028](docs/adr/0028-ux-journey-testing-protocol.md) method) — folded into the 0019/0022/0023/0025 addenda; tactical history in git. Security & hardening folded into #50 (2026-06-21) — stored-XSS textbox sink sanitised via DOMPurify ([ADR 0029](docs/adr/0029-sanitize-user-authored-html.md)) + import-time defense-in-depth, single-pass HTML strips → fixpoint `stripHtmlTags`, model-array import bounds (oversized import rejected via §6.3 notification), `npm audit fix` lockfile (prod deps 0 HIGH; dev-only wrangler `ws`/`undici` HIGH deferred to a major bump), CodeQL `actions` language + dead-export knip fix; git history. **Labels & text-styling productization shipped (2026-07-05, `integration`→`master` PR):** floating Label entity, docked style strip as the sole styling surface, node/connector name↔label decouple, inline canvas text editing + dual-scope strip formatting + Docs-style link cards, rectangle/text-box borders + opacity, rotate handles — see [ADRs 0030–0034](docs/adr/) (+ ADR 0020/0026 addenda) and the (retired) `docs/tactical/integration-productization-plan.md` git history. **On-canvas per-node icon resize shipped (2026-07-19):** node transform handles + per-node `viewItem.iconScale`, "trace each shape" selection/hover rim (screen box for standing isometric icons / iso diamond for flat elements + Material icons), top-bar Icon-size slider removed — see [ADR 0044](docs/adr/0044-on-canvas-icon-resize.md) (Accepted; supersedes [ADR 0030](docs/adr/0030-docked-style-controls-strip.md) §1 icon-size) + git history. **Off-grid rendered-geometry hardening shipped (2026-07-23):** one `utils/renderedGeometry.ts` source of truth consumed by every renderer/chrome/hit-test path (a source-scan contract test fails CI on a hand-rolled composition), an invariant suite asserting render == chrome == hit zone across element kind × offset corpus × canvas mode, and real-mouse e2e for the sub-tile regime — see the [ADR 0023](docs/adr/0023-off-grid-positioning-and-collision.md) addendum + [docs/guidelines/testing.md](docs/guidelines/testing.md) (this file's git history for the plan). **Open question:** turning the global snap toggle ON does not re-snap items that are already off-grid; today's behaviour is frozen by test (`snap-grid.spec.ts`) pending a product decision. |
| **ENG-PAN** | Engine perf — large-diagram **pan** floor (R1) + harness gap | `[x]` | ⚠️ Very High | NEW post-T2 sub-track, absent from the original charter. Shipped: #54 synchronous canvas repaint; `3cf76c08` decoupled viewport culling from the pan frame. **CLOSED 2026-07-08 by ENG-T4, not by the planned fix.** The WebGL2 substrate made pan **O(1) on the CPU** (60 fps to ~20,000 nodes), so the R1 repaint floor dissolved — the dirty-region / hybrid redraw was never built and is not needed. The `fix/large-diagram-pan-perf` branch is gone and the [known_issues.md](known_issues.md) R1 entry is now marked resolved (it stayed open for a week after the substrate landed; closed 2026-07-15). *(`measurePan` did land in the harness, so the floor is measurable regardless.)* |
| **ENG-T3** | Engine perf — simulation engine (LEB60) | `[ ]` 🔭 **DEFERRED — unratified** | ⚠️ Very High | **Deferred, pending product ratification.** ECS + fixed-timestep tick loop + spatial-hash collision, tick decoupled from render.<br><br>**The product mandate (recorded here 2026-07-15 — this row is now its only home).** T3 exists to serve a *"kids' tile-world sim"* target: **1,000 animals** moving 1 tile/tick @ 10 ticks/s with collision, atop **2,000 structures** on a **256×256** grid → p95 frame ≤ 16.6 ms; tick p95 < 5 ms; render p95 < 11 ms; tick and render never serialising into one >50 ms long task; per-tick input→output rule eval (e.g. cow+grass→…, cow+bull→calf adjacency) inside the tick budget. **This mandate is not ratified in any vision doc, ADR, or product register** — Axoview ships as a static diagram editor. It is recorded, not endorsed.<br><br>**Gate (updated 2026-07-15).** The original trigger — "a measured **Canvas2D** ceiling" — is **void**: Canvas2D is gone ([ADR 0038](docs/adr/0038-webgl-instanced-render-substrate.md)). T3 reopens only when **(a)** the sim product is ratified **and (b)** a tick-workload harness measures a real ceiling *on the WebGL2 substrate*. **LEB60 is still unmeasured** — the substrate now sustains SSB ≈ 20,000 *static* nodes, but no tick harness exists, so the *moving*-entity budget is unknown. Building that harness is the first step, not the ECS.<br><br>Building blocks already shipped: `TileIndex` ([ADR 0021](docs/adr/0021-paste-algorithmic-perf-and-spatial-index.md)), the honest connector-paint anti-cheat (ENG-T2.5), the GPU substrate (ENG-T4), and the harness + measurement protocol ([ADR 0020](docs/adr/0020-engine-perf-harness-and-measurement-protocol.md), which holds the durable tier ladder + LEB60 definition). If T3 is ratified it earns its own ADR; if it is dropped, say so here and the last reason to remember it is gone. |
| **ENG-T4** | Engine perf — WebGL2 instanced render | `[x]` **SHIPPED 2026-07-08** | ⚠️ Very High | **Reverses the 2026-06-25 "no rewrite, stay on Canvas2D" decision.** WebGL2 `glSpriteBatch` is now the **sole** bulk substrate — nodes, labels, connector and rectangle bodies drawn as one `drawArraysInstanced` per layer per frame from one atlas, tile→screen transform in the vertex shader → **O(1) CPU pan, 60 fps to ~20,000 nodes**. Canvas2D/DOM bulk fallback removed; `WebGLUnsupportedScreen` gates unsupported browsers; ADR 0019 superseded in part; ADR 0020 amended (anti-cheat re-pointed at GPU `drawCount`). Shipped v3.5.0 / PR #63, with GPU line-styles + fidelity follow-ups in v3.6.0. See [ADR 0038](docs/adr/0038-webgl-instanced-render-substrate.md), the scoped 2026-07-08 review (retired — git history), and [canvas-rendering-guidelines.md](docs/guidelines/canvas-rendering-guidelines.md) (the fidelity contract — **CI cannot see GPU output; every visual change needs a real-browser check**). |
| **QA-EXPLORE** | Exploratory testing program — campaign + remediation + standing agent | `[x]` **SHIPPED 2026-08-08 · finalized 2026-08-10** | ⚠️ Very High | Exploratory remediation shipped (240 campaign bugs, 22 owner rulings, `/explore` agent) — see [ADR 0047](docs/adr/0047-exploratory-testing-program.md) and this file's git history. A hypothesis-and-verification campaign probed **385 falsifiable hypotheses across 27 areas and confirmed 240 bugs while every suite was green** — each finding outside existing coverage by construction of the novelty rule, each carrying a committed expected-fail repro in a CI-quarantined probe lane. Remediation ran as waves 0–6: trust & security, interaction & rendering correctness, consistency & decided UX, then the two design-gated larges — page-stamped history ([ADR 0038](docs/adr/0038-webgl-instanced-render-substrate.md) §8) and **the canvas merge**, which collapsed four bulk WebGL2 contexts into one sorted draw so `zIndex` and `layer.order` finally cross an entity type. Wave 6 closed the picker's matching gap (it now resolves the layer tier *and* cross-type through the renderer's own comparator), froze the record to [docs/reviews/exploratory-2026-07.md](docs/reviews/exploratory-2026-07.md), and turned the method into the standing [`/explore`](.claude/commands/explore.md) skill — delta-scoped against the last-swept commit, runnable headless. A **mop-up wave (7, 2026-08-10)** then closed the 18 out-of-cluster entries the cluster-waves had left silently open and **dissolved the standing probe lane** — `__explore__/` and `tests-exploratory/` deleted, surviving open-bug repros now colocated `test.fail()`/`it.failing` tests CI runs — for a **final ledger of 204 filed entries, all fixed except 4 accepted-open by owner ruling** (GPU-15 cosmetic, RND-07 needs a new surface, ICON-08 an ADR 0044 §6 trade-off, VIEW-09a dead-code); see ADR 0047's 2026-08-10 addendum. **Durable lesson worth more than the fix list:** the campaign's *evidence* was reliable and its *diagnoses* were hypotheses — a dozen recorded root causes were corrected while being fixed. |
...
- Productization audit (2026-03 → 2026-05) complete — see ADRs 0008–0011 + `docs/reviews/technical-review-2026-05.md` (retired — git history) for the durable record. v1.0.0 shipped 2026-05-23; M0–M10 milestone arc closed via 17 locked decisions across cleanup execution, ADR-implementation, deployment hardening, CI/automation hardening, and E2E rewrite. Sibling tacticals `e2e-suite-rewrite.md` + `git-automation-hardening.md` retained as session telemetry; `flare_plan.md` retired pre-audit (decisions absorbed into ADRs 0009 + 0010).
...
- **v1.1 tech-debt cleanup** — shipped 2026-05-23 → 2026-05-25 across PRs #3–#7. Track 0 (dead-code wave, ~9.4k LOC removed across 8 clusters), Track 1 (audit-truth fixes — nginx security headers, `isPublicRoute` alignment, ADR 0005 amendment, web-vitals dep removal, i18n `mainMenu` cascade), Track 2 (productization decisions — backend `npm ci` at build time + committed lockfile, drop `NPM_TOKEN` from `release.yml`, drop Node 20 from CI matrix, compose `name: axoview` + service rename `axoview` → `app`), Track 3 (`docs/technical-review-2026-05.md` §12 corrections appendix — B2 / B5 / M6 / M7 / anomaly #31 / published-posture / version notes), Track 4 (CodeQL toggle + master branch protection + single-tenant deployment callout). Mid-cleanup discovery: lasso connector-delete regression + path-hit selection semantics fixed in PR #6 (`2ed5f79`). Tracks 5 (test coverage incl. canvas cross-interaction E2E), 6 (refactor candidates for files > 300 LOC — **superseded 2026-05-26 by Sonar-driven refactor priorities; see catalogued workstream below**), and 7 (new audit workstreams incl. `no-explicit-any` baseline cleanup) catalogued; each spawns its own tactical when authorized. Durable corrections record lives in `docs/reviews/technical-review-2026-05.md` §12 (retired — git history). Tactical `docs/tactical/v1.1-tech-debt.md` retired per convention.

**Deferred features:**

- **Rich-text node caption on the Canvas2D layer — RETIRED (2026-06-30, ADR 0032).** The on-canvas rich caption is gone: the node identity model is now `name`-only on the canvas, with the rich `description` folded into **Notes** (full retire / pure Option A — see [`docs/adr/0032-node-name-caption-label-model.md`](docs/adr/0032-node-name-caption-label-model.md)). Near-node annotation is served by the floating **Label** (ADR 0031). No styled-run Canvas2D text renderer is needed — that work is intentionally avoided to protect the large-diagram pan-perf budget.

  Prior register entry — ADR 0011 failure-of-intent Dialogs — shipped in PR #27 (2026-06-10).

- **Labels & text-styling productization — fast-follow items (2026-07-05).** The full deferred catalogue lives in the retired [`docs/tactical/integration-productization-plan.md`](docs/tactical/) git history. Carried forward with real forcing functions: **(a)** connector add-label length-cap divergence (the F2 + context-menu writers are uncapped vs the schema `.max(256)` — *already drifted once*); **(b)** the e2e regression-coverage backlog (image-export label legibility, connector Details "Add label" fall-through, "Add note" on all element types, text-color / no-color-border picker) is catalogued in [testing.md § Known Coverage Gaps](docs/guidelines/testing.md). Diagram-to-diagram links extend to **node only** (connectors / floating Labels deferred — need a `link` field + view-mode nav on those types). Everything else was dropped with rationale per `feedback_catalogue_is_debt` (git history).

---

**Catalogued workstreams (authorized engineering; each spawns its own tactical when its gate opens — `feedback_okr_prompt_header.md` discipline). These are the only catalogued survivors of the v1.1 closure prune; default disposition for new findings is DROP, not catalogue (`feedback_catalogue_is_debt.md`).**
...
- ~~**XSS / CSP hardening**~~ — **mostly discharged 2026-06-21.** The Quill `dangerouslySetInnerHTML` sink in `TextBox.tsx` is sanitised via DOMPurify ([ADR 0029](docs/adr/0029-sanitize-user-authored-html.md)) with import-time defence-in-depth, and its gate ("before any shared / multi-user content") has since **opened** — Drive-native sharing ([ADR 0042](docs/adr/0042-drive-native-sharing-and-readonly-preview.md)) ships shared content. *Still open:* CSP `'unsafe-inline'` style-src tightening only. Source: §6d/§11.
- ~~**Privacy disclosure + policy**~~ — **discharged 2026-07-07.** Drive reached production as v3.2.0; the PR #59 master-merge owner checklist (OAuth consent-screen publish + privacy policy) was cleared, and `/privacy` + `/terms` pages shipped (PR #59/#60, linked from the empty-state footer across 13 locales), satisfying ADR 0036's "Drive does not ship to production without it" criterion. Source: §8/§11.
