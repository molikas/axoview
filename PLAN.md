...
> Last updated: 2026-06-25 (T1+T2 perf banked & merged to master; canvas-UX Track P gate passed. NEW active sub-track: large-diagram **pan** floor (R1). Decision 2026-06-25: **continue evolving Canvas2D, no engine rewrite** — T3 simulation / T4 WebGL **deferred** pending product ratification + a measured ceiling. See `docs/tactical/perf-charter.md` + `perf-results/decision-log.md`.)
...
| **ENG-PAN** | Engine perf — large-diagram **pan** floor (R1) + harness gap | `[~]` | ⚠️ Very High | NEW post-T2 sub-track (branch `fix/large-diagram-pan-perf`), absent from the original charter. Shipped: #54 synchronous canvas repaint (killed the canvas-trails-DOM rubber-band introduced when canvas became the bulk renderer); `3cf76c08` decoupled viewport culling from the pan frame (removed per-tile-crossing reconcile + O(N) visible-array churn). **OPEN (R1, [known_issues.md](known_issues.md)):** per-frame synchronous canvas-repaint floor — ~24–55 fps on AC, ~6–8 fps CPU-throttled (~54-node scene). Fix = dirty-region/layered redraw **or** sync-small/async-large hybrid. Two prereqs that don't exist yet: a large-N `NodesCanvas.scrollSync` guard variant + a `measurePan` harness scenario (**the perf harness has no pan coverage** — single biggest measurement gap). Scope: [perf-results/pan-r1-design.md](perf-results/pan-r1-design.md). |
| **ENG-T3** | Engine perf — simulation engine (LEB60) | `[ ]` 🔭 **DEFERRED** | ⚠️ Very High | **Deferred 2026-06-25, pending product ratification.** ECS + fixed-timestep tick + spatial-hash collision; tick decoupled from render; target 1,000 moving entities @60fps. The LEB60 "moving-entity / kids' tile-world sim" mandate that justifies T3 (and T4) lives **only** in `perf-charter.md` — not ratified in any vision doc, ADR, or product register; Axoview ships as a static diagram editor (Canvas2D is sub-linear + content-agnostic to ~2,000 nodes). Stays deferred until **(a)** the sim product is ratified **and (b)** a tick-workload harness measures a real Canvas2D ceiling (charter RED gate). Building blocks already shipped: `TileIndex` ([ADR 0021](docs/adr/0021-paste-algorithmic-perf-and-spatial-index.md)), honest connector-paint anti-cheat (ENG-T2.5). |
| **ENG-T4** | Engine perf — WebGL instanced render (stretch) | `[ ]` 🔭 **DEFERRED** | — | Stretch tier; only reached if a **measured** Canvas2D / per-tick-redraw ceiling forces it **and** the sim product is ratified. No design beyond the charter tier ladder. Full-engine-rewrite (replace the isoflow-fork engine) was evaluated 2026-06-25 and **declined** for now — unjustified rebuild risk against an unratified mandate, and the e2e suite under-specifies the visual/geometry contract (no visual-regression baseline). See the 2026-06-25 resume point in `perf-results/decision-log.md`. |
