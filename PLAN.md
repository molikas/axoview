...
- **v1.1 tech-debt cleanup** — shipped 2026-05-23 → 2026-05-25 across PRs #3–#7. Track 0 (dead-code wave, ~9.4k LOC removed across 8 clusters), Track 1 (audit-truth fixes — nginx security headers, `isPublicRoute` alignment, ADR 0005 amendment, web-vitals dep removal, i18n `mainMenu` cascade), Track 2 (productization decisions — backend `npm ci` at build time + committed lockfile, drop `NPM_TOKEN` from `release.yml`, drop Node 20 from CI matrix, compose `name: axoview` + service rename `axoview` → `app`), Track 3 (`docs/technical-review-2026-05.md` §12 corrections appendix — B2 / B5 / M6 / M7 / anomaly #31 / published-posture / version notes), Track 4 (CodeQL toggle + master branch protection + single-tenant deployment callout). Mid-cleanup discovery: lasso connector-delete regression + path-hit selection semantics fixed in PR #6 (`2ed5f79`). Tracks 5 (test coverage incl. canvas cross-interaction E2E), 6 (refactor candidates for files > 300 LOC), and 7 (new audit workstreams incl. `no-explicit-any` baseline cleanup) catalogued; each spawns its own tactical when authorized. Durable corrections record lives in [`docs/technical-review-2026-05.md`](docs/technical-review-2026-05.md) §12. Tactical `docs/tactical/v1.1-tech-debt.md` retired per convention.

**Catalogued next-workstreams (each spawns its own tactical when authorized — see `feedback_okr_prompt_header.md` discipline):**

- **v1.1 `no-explicit-any` baseline elimination** *(authorized 2026-05-25; tactical pending spawn)* — Source: ~20 `no-explicit-any` warnings surfaced as CI annotation noise in PRs #5 / #6 (`App.tsx:214` × 3 sites, `AppToolbar.tsx:297` × 1 site, `DiagnosticsOverlay.tsx` × 6 sites, plus residue elsewhere). **Suppression is the wrong fix** — each site needs proper typing (concrete types where shape is known; narrow `unknown` + type-guards where dynamic). Promotion candidate: enable `@typescript-eslint/no-explicit-any: error` once baseline hits zero. **Do NOT bundle as Tier 3 polish** — proper typing requires per-site investigation. Initiative shape: per-package sweep (likely 1 PR per workspace, or one bundled if scope stays small). This row originally lived in the retired `v1.1-tech-debt.md` Track 7 catalogue; lost in the PR #7 / PR #8 merge-conflict resolution; restored here as the durable home.
- **v1.1 Cloudflare hardening** *(authorized 2026-05-25; tactical pending spawn)* — Source: 30-day CF Analytics review (789 4xx / 557 5xx; 10.49% error rate; ~100% vulnerability-scanner traffic). Two real issues distinguished from bot noise: (1) **origin emitting 500 on probe paths** (`/.env`, `/.docker/secrets.json`, `/api/admin/role/id`, etc.) — risk of stack-trace / framework-version leakage; on our serverless topology this is Worker `app.ts` auth-middleware or 503 catch-all throwing on specific inputs OR SPA fallback boot-path throwing on non-`/api/*` garbage, NOT a traditional origin server; (2) **522 Origin Connection Timeouts** (338 occurrences) — on a CF Pages + Functions deployment usually means Function cold-start exceeds the edge timeout window; needs separate diagnosis. Initiative shape: Workstream A (origin diagnosis — P1 500 root cause + P2 522 timeout cross-reference, ~1 session each); Workstream B (CF edge hardening — Bot Fight Mode + WAF Managed Rules + scanner-path block rule + rate-limit, ~1 session; reversible; MUST follow A so WAF doesn't mask the 500 diagnosis); Hygiene (P4 exposure-verification checklist, <30 min). **Pre-spawn fixes to remember**: (a) the proposed WAF rule `contains ".env"` would block legitimate paths like `/environment`; use `ends_with(".env")` or path-exact-match instead. (b) Workstream A's 500-on-`/api/*` diagnosis naturally piggybacks on v1.1-test-coverage 5a Commit 7 (`app.ts surface smoke`) — sequence after 5a ships so findings inform the tactical. Authorization gate when ready: *"Spawn the orchestrator subagent for v1.1 Cloudflare hardening. Source: this PLAN.md entry + raw CF Analytics dashboard data the user will paste. Produce tactical + milestone path + first session prompt; tier 1 = Workstream A (origin diagnosis), tier 2 = Workstream B (edge hardening), tier 3 = P4 hygiene."*
