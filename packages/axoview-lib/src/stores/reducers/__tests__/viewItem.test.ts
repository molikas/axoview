  ...
  getConnectorsByViewItem: jest.fn(
    (viewItemId: string, connectors: Connector[]) => {
      return connectors.filter((connector) =>
        connector.anchors.some((anchor: any) => anchor.ref?.item === viewItemId)
      );
    }
  )
}));
...
describe('viewItem reducer', () => {
  ...
  describe('deleteViewItem', () => {
    it('should delete a view item and its associated connectors', () => {
      ...
      expect(
        result.model.views[0].items.find((item) => item.id === 'item1')
      ).toBeUndefined();
  ...
  describe('updateViewItem', () => {
    it('should update view item properties', () => {
      ...
      const updatedItem = result.model.views[0].items.find(
        (item) => item.id === 'item1'
      );
      expect(updatedItem?.tile).toEqual({ x: 2, y: 2 });
      expect(updatedItem?.zIndex).toBe(5);
    });
  ...
  describe('edge cases and state immutability', () => {
    ...
    it('should handle multiple operations in sequence', () => {
      ...
      expect(
        result.model.views[0].items.find((item) => item.id === 'item3')
      ).toBeDefined();
      expect(
        result.model.views[0].items.find((item) => item.id === 'item3')?.zIndex
      ).toBe(99);
      expect(
        result.model.views[0].items.find((item) => item.id === 'item1')
      ).toBeUndefined();
  ...
  describe('batch-delete cascade (deleteSelectedItems pattern)', () => {
    ...
    it('connector shared by two deleted items is not double-counted', () => {
      ...
      const remainingConnectorIds =
        result.model.views[0].connectors?.map((c) => c.id) ?? [];
    ...
    it('unrelated connector survives batch delete of two items', () => {
      ...
      mockState.scene.connectors['connectorStandalone'] = {
        path: {
          tiles: [],
          rectangle: { from: { x: 10, y: 10 }, to: { x: 11, y: 11 } }
        }
      };
      ...
      expect(result.model.views[0].connectors).toHaveLength(1);
      expect(result.model.views[0].connectors![0].id).toBe(
        'connectorStandalone'
      );
    });
  });
