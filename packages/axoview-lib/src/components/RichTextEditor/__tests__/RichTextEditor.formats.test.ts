...
// The mocked default export needs the Quill static: RichTextEditor registers
// the align style attributor at module scope (ADR 0034 addendum 2026-07-03).
jest.mock('react-quill-new', () => ({
  __esModule: true,
  default: Object.assign(() => null, {
    Quill: { register: () => {}, import: () => ({}) }
  })
}));
...
describe('RichTextEditor — Quill formats config', () => {
  ...
  it('contains all expected base formats', () => {
    const expected = [
      ...
      'header',
      'list',
      // ADR 0034 addendum 2026-07-03 — paragraph alignment (style attributor).
      'align',
      // ADR 0034 addendum 2026-07-04 — per-range text color.
      'color',
      'blockquote',
      'code-block'
    ];
  ...
  it('has exactly 11 formats (pin count to catch unreviewed additions)', () => {
