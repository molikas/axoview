...
describe('buildLinkShortcutBinding', () => {
  ...
  it('keeps a non-empty selection as-is and fires the edit-at-selection event', () => {
    const binding = buildLinkShortcutBinding();
    const { quill, calls } = makeQuill('visit docs today\n', null);
    const seen = jest.fn();
    window.addEventListener(EDIT_LINK_AT_SELECTION_EVENT, seen);
    const result = binding.handler.call({ quill }, { index: 6, length: 4 });
    window.removeEventListener(EDIT_LINK_AT_SELECTION_EVENT, seen);
  ...
  it('expands a collapsed caret to the word under it before opening', () => {
    const binding = buildLinkShortcutBinding();
    const { quill, calls } = makeQuill('visit docs today\n', null);
    const seen = jest.fn();
    window.addEventListener(EDIT_LINK_AT_SELECTION_EVENT, seen);
  ...
  it('consumes the key WITHOUT opening when the caret sits in whitespace', () => {
    const binding = buildLinkShortcutBinding();
  ...
  it('falls back to getSelection when the binding range is null', () => {
    const binding = buildLinkShortcutBinding();
