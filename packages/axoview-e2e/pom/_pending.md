...
| 24 | `strip-link-diagram-picker` + `strip-link-diagram-listbox` | `axoview-lib/.../TopBarStyleControls/TopBarStyleControls.tsx` | 6 | J5 (multi-diagram.spec) — DiagramLinkPOM.openPicker |
| 25 | `strip-link-diagram-open` | `axoview-lib/.../TopBarStyleControls/TopBarStyleControls.tsx` | 6 | J5 (multi-diagram.spec) — DiagramLinkPOM.clickOpenLinkedDiagram |
...
(multi-diagram node deck; dialogs Settings/Help/LeftDock/BottomDock;
...
| ~~`DiagramLinkPOM`~~ | ~~6 (multi-diagram)~~ | ~~`setLinkedDiagram(name)`, `clickOpenLinkedDiagram()`, `setNodeName(text)`~~ | **✅ authored Session 6 (J5) as `pom/DiagramLinkPOM.ts`** — covers `linkPickerInput()`, `linkPickerListbox()`, `openLinkedDiagramButton()`, `expectVisible()`, `openPicker()`, `getOptionNames()`, `selectLinkedDiagram(name)`, `clickOpenLinkedDiagram()`. `setNodeName(text)` deferred (no J5 sub-test needs it; lazy retrofit on the name TextField waits for a consuming spec). |
...
- `[data-testid="connector-path"]`, `[data-testid="node-label"]`, `[data-testid="node-header-link"]`, `[data-testid="lasso-selection"]`, `[data-testid="item-controls-panel"]`, `[data-testid="context-menu"]`, `[data-testid="export-svg-button"]`, `[data-testid="strip-link-diagram-open-diagram"]`, `[data-testid="node-panel-*"]` — sub-canvas / panel affordances; each gets a POM-owned `data-axoview-id` in the session that introduces the consuming spec.
